...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Master / Hotel']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Master / Hotel')]); ?>
  <div x-data="hotelCrud()" x-init="init()" class="space-y-4 overflow-x-hidden self-start w-full">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'master.hotel.create')): ?>
              ...
              <tr class="border-t border-slate-100">
                ...
                <td class="px-4 py-3">
                  <div class="flex justify-end gap-2">
                    ...
                    <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'master.hotel.edit')): ?>
                    ...
                    <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'master.hotel.delete')): ?>
      ...
      return {
        ...
        async fetchData(){
          ...
          try {
            ...
            const response = await fetch(`<?php echo e(route('admin.inventaris.hotel.data')); ?>?${params}`);
        ...
        async fetchUserOutlets(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.hotel.user-outlets")); ?>');
            const data = await response.json();
        ...
        async fetchCities(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.hotel.cities")); ?>');
            const data = await response.json();
            this.cities = data;
          } catch (error) {
        ...
        async openEdit(h){ 
          try {
            const response = await fetch(`<?php echo e(route('admin.inventaris.hotel.show', '')); ?>/${h.id}`);
            const data = await response.json();
        ...
        async submitForm(){
          ...
          try {
            const url = this.form.id 
              ? `<?php echo e(route('admin.inventaris.hotel.update', '')); ?>/${this.form.id}`
              : '<?php echo e(route("admin.inventaris.hotel.store")); ?>';
        ...
        async deleteNow(){
          ...
          this.deleting = true;
          try {
            const response = await fetch(`<?php echo e(route('admin.inventaris.hotel.destroy', '')); ?>/${this.toDelete.id}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                'Content-Type': 'application/json'
        ...
        async fetchRoomTypes(hotelId){
          this.roomTypes = [];
          try {
            const baseUrl = '<?php echo e(url("admin/inventaris/hotel")); ?>';
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\inventaris\hotel\index.blade.php ENDPATH**/ ?>