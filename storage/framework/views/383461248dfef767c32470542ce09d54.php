...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Laporan Laba Rugi']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Laporan Laba Rugi')]); ?>
    ...
    function profitLossManagement() {
      return {
        routes: {
          ...
          profitLossData: '<?php echo e(route("finance.profit-loss.data")); ?>',
          exportXLSX: '<?php echo e(route("finance.profit-loss.export.xlsx")); ?>',
          exportPDF: '<?php echo e(route("finance.profit-loss.export.pdf")); ?>'
        },
        filters: {
          outlet_id: '',
          period: 'monthly',
              ...
              html += `
                <tr class="border-t border-slate-100 hover:bg-slate-50">
                  <td class="px-4 py-2 border-r border-slate-100">
                    <span class="font-mono text-xs text-blue-600 font-semibold">${account.code || '-'}</span>
                  </td>
                  <td class="px-4 py-2 border-r border-slate-100">
                    <span class="text-slate-800">${account.name || 'Unnamed Account'}</span>
                  </td>
                  <td class="px-4 py-2 text-right">
                    <span class="text-green-600 font-semibold">${this.formatCurrency(account.amount)}</span>
                  </td>
                </tr>
              `;
                ...
                account.children.forEach((child) => {
                  html += `
                    <tr class="border-t border-slate-50 bg-slate-25 hover:bg-slate-50">
                      ...
                      <td class="px-4 py-2 border-r border-slate-100">
                        <span class="text-slate-600 text-sm">${child.name || 'Unnamed Account'}</span>
                      </td>
                      <td class="px-4 py-2 text-right">
                        <span class="text-green-600 text-sm">${this.formatCurrency(child.amount)}</span>
                      </td>
                    </tr>
                  `;
                });
            ...
            this.profitLossData.expense.accounts.forEach((account) => {
              html += `
                <tr class="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td class="px-4 py-2 text-right">
                    <span class="text-red-600 font-semibold">${this.formatCurrency(account.amount)}</span>
                  </td>
                </tr>
              `;
                ...
                account.children.forEach((child) => {
                  html += `
                    <tr class="border-t border-slate-50 bg-slate-25 hover:bg-slate-50">
                      ...
                      <td class="px-4 py-2 text-right">
                        <span class="text-red-600 text-sm">${this.formatCurrency(child.amount)}</span>
                      </td>
                    </tr>
                  `;
                });
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\finance\labarugi\index_new.blade.php ENDPATH**/ ?>