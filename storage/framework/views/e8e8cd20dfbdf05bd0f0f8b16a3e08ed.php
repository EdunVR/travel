...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'SDM / Manajemen Kinerja']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('SDM / Manajemen Kinerja')]); ?>
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
                ...
                <input type="month" id="period-filter" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="<?php echo e(date('Y-m')); ?>">
            </div>
    ...
    <div class="relative top-10 mx-auto p-5 border w-full max-w-3xl shadow-lg rounded-lg bg-white mb-10">
        ...
        <form id="appraisal-form" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <?php else: ?>
                <input type="hidden" id="outlet-id" value="<?php echo e($outlets[0]->id_outlet); ?>">
                <?php endif; ?>
                ...
                <div>
                    ...
                    <input type="month" id="period" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="<?php echo e(date('Y-m')); ?>">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Penilaian <span class="text-red-500">*</span></label>
                    <input type="date" id="appraisal-date" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="<?php echo e(date('Y-m-d')); ?>">
                </div>
            </div>
...
<script>
let dataTable;
const baseUrl = '<?php echo e(route("sdm.kinerja.index")); ?>';
...
function loadData() {
    ...
    fetch(`<?php echo e(route('sdm.kinerja.data')); ?>?outlet_filter=${outletFilter}&period_filter=${periodFilter}&status_filter=${statusFilter}&employee_filter=${employeeFilter}`)
...
function loadStatistics() {
    ...
    fetch(`<?php echo e(route('sdm.kinerja.statistics')); ?>?period_filter=${periodFilter}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function loadEmployees(outletId = null) {
    const url = outletId 
        ? `<?php echo e(route('sdm.kinerja.employees')); ?>?outlet_id=${outletId}`
        : `<?php echo e(route('sdm.kinerja.employees')); ?>`;
...
function openAddModal() {
    ...
    document.getElementById('appraisal-date').value = '<?php echo e(date("Y-m-d")); ?>';
    document.getElementById('period').value = '<?php echo e(date("Y-m")); ?>';
    document.getElementById('form-modal').classList.remove('hidden');
}

function editAppraisal(id) {
    fetch(`<?php echo e(route('sdm.kinerja.index')); ?>/${id}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function handleSubmit(e) {
    ...
    const url = id 
        ? `<?php echo e(route('sdm.kinerja.index')); ?>/${id}`
        : `<?php echo e(route('sdm.kinerja.store')); ?>`;
...
function exportPdf() {
    ...
    window.open(`<?php echo e(route('sdm.kinerja.export.pdf')); ?>?outlet_filter=${outletFilter}&period_filter=${periodFilter}&status_filter=${statusFilter}`, '_blank');
}

function exportSinglePdf(id) {
    window.open(`<?php echo e(route('sdm.kinerja.export.pdf')); ?>?id=${id}`, '_blank');
}
</script>
<?php $__env->stopPush(); ?>
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\sdm\kinerja\index-old.blade.php ENDPATH**/ ?>