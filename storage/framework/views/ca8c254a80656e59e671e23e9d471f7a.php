...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Master / Transportasi Saudi']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Master / Transportasi Saudi')]); ?>
      ...
      return {
        ...
        async fetchData() {
          ...
          try {
            ...
            const res = await fetch(`<?php echo e(route('admin.inventaris.transport.data')); ?>?${params}`);
        ...
        async openEdit(t) {
          const res = await fetch(`<?php echo e(route('admin.inventaris.transport.show', '')); ?>/${t.id}`);
          const data = await res.json();
        ...
        async submitForm() {
          ...
          try {
            const url = this.form.id ? `<?php echo e(route('admin.inventaris.transport.update', '')); ?>/${this.form.id}` : '<?php echo e(route("admin.inventaris.transport.store")); ?>';
            const method = this.form.id ? 'PUT' : 'POST';
            const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' }, body: JSON.stringify(this.form) });
        ...
        async deleteNow() {
          this.deleting = true;
          try {
            const res = await fetch(`<?php echo e(route('admin.inventaris.transport.destroy', '')); ?>/${this.toDelete.id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' } });
            const result = await res.json();
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\inventaris\transport\index.blade.php ENDPATH**/ ?>