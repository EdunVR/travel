...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Penggajian / Payroll']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Penggajian / Payroll']); ?>
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div class="flex gap-2">
                <a href="<?php echo e(route('sdm.payroll.coa.index')); ?>" class="px-4 py-2 bg-slate-600 text-white rounded-lg hover:bg-slate-700 flex items-center gap-2">
                    <i class='bx bx-cog'></i>
                    <span>Setting COA</span>
                </a>
                <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'hrm.payroll.create')): ?>
        ...
        <div class="bg-white rounded-xl shadow-card p-4 border border-slate-200">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div>
                    ...
                    <input type="month" id="periodFilter" value="<?php echo e(date('Y-m')); ?>" class="w-full px-3 py-2 border border-slate-300 rounded-lg">
                </div>
                <div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#period').val('<?php echo e(date("Y-m")); ?>');
            $('#payment_date').val('<?php echo e(date("Y-m-d")); ?>');
        ...
        async function loadData() {
            try {
                ...
                const response = await fetch(`<?php echo e(route('sdm.payroll.data')); ?>?${params}`);
        ...
        async function loadEmployees() {
            ...
            try {
                const response = await fetch(`<?php echo e(route('sdm.payroll.employees')); ?>?outlet_id=${outletId}`);
                const result = await response.json();
        ...
        function loadAttendanceData() {
            ...
            $.ajax({
                url: '<?php echo e(route("sdm.payroll.attendance.summary")); ?>',
                method: 'GET',
                data: {
        ...
        function renderTable() {
            ...
            const rows = payrolls.map(p => {
                ...
                if (p.status === 'draft') {
                    actions = `
                        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'hrm.payroll.edit')): ?>
                        <button onclick="editPayroll(${p.id})" class="px-2 py-1 text-blue-600 hover:bg-blue-50 rounded" title="Edit">
                            <i class='bx bx-edit'></i>
                        </button>
                        <?php endif; ?>
                        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'hrm.payroll.approve')): ?>
                        <button onclick="approvePayroll(${p.id})" class="px-2 py-1 text-green-600 hover:bg-green-50 rounded" title="Approve">
                            <i class='bx bx-check'></i>
                        </button>
                        <?php endif; ?>
                        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'hrm.payroll.delete')): ?>
        ...
        async function editPayroll(id) {
            try {
                const response = await fetch(`<?php echo e(route('sdm.payroll.index')); ?>/${id}`);
                const result = await response.json();
        ...
        $('#payrollForm').on('submit', async function(e) {
            ...
            try {
                const url = id 
                    ? `<?php echo e(route('sdm.payroll.index')); ?>/${id}`
                    : `<?php echo e(route('sdm.payroll.store')); ?>`;
        ...
        async function approvePayroll(id) {
            ...
            try {
                const response = await fetch(`<?php echo e(route('sdm.payroll.index')); ?>/${id}/approve`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
                    }
        ...
        async function payPayroll(id) {
            ...
            try {
                const response = await fetch(`<?php echo e(route('sdm.payroll.index')); ?>/${id}/pay`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
                    }
        ...
        function printSlip(id) {
            window.open(`<?php echo e(route('sdm.payroll.index')); ?>/${id}/slip`, '_blank');
        }
        ...
        function exportPdf() {
            ...
            window.open(`<?php echo e(route('sdm.payroll.export.pdf')); ?>?${params}`, '_blank');
        }
        ...
        function exportExcel() {
            ...
            window.location.href = `<?php echo e(route('sdm.payroll.export.excel')); ?>?${params}`;
        }
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\sdm\payroll\index.blade.php ENDPATH**/ ?>