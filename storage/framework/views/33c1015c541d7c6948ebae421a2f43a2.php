...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Inventaris / Transfer Gudang']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Inventaris / Transfer Gudang')]); ?>
  <div x-data="transferGudang()" x-init="init()" class="space-y-4 overflow-x-hidden">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.transfer-gudang.create')): ?>
                      ...
                      <div class="flex flex-wrap gap-1">
                        <template x-if="request.status_raw === 'menunggu'">
                          <div class="flex gap-1">
                            <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.transfer-gudang.approve')): ?>
      ...
      return {
        ...
        async loadRequests(){
          ...
          try {
            ...
            const response = await fetch(`<?php echo e(route("admin.inventaris.transfer-gudang.data")); ?>?${params}`);
        ...
        async loadPendingCount(){
          try {
            const response = await fetch(`<?php echo e(route("admin.inventaris.transfer-gudang.data")); ?>?status=menunggu`);
            const data = await response.json();
        ...
        async approveTransfer(requestId){
          ...
          try {
            ...
            const response = await fetch(`<?php echo e(route("admin.inventaris.transfer-gudang.approve", ":id")); ?>`.replace(':id', requestId), {
              method: 'POST',
              headers: {
        ...
        async rejectTransfer(requestId){
          ...
          try {
            // PERBAIKAN: Gunakan route yang benar tanpa empty string
            const response = await fetch(`<?php echo e(route("admin.inventaris.transfer-gudang.reject", ":id")); ?>`.replace(':id', requestId), {
              method: 'POST',
              headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                'Content-Type': 'application/json'
        ...
        async loadOutlets(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.transfer-gudang.outlets")); ?>');
            const data = await response.json();
            this.outlets = data;
          } catch (error) {
        ...
        async loadSenderItems(){
          ...
          try {
            const response = await fetch(`<?php echo e(route("admin.inventaris.transfer-gudang.items")); ?>?outlet_id=${this.sender}&type=${this.tab}`);
            const data = await response.json();
        ...
        async loadReceiverItems(){
          ...
          try {
            const response = await fetch(`<?php echo e(route("admin.inventaris.transfer-gudang.items")); ?>?outlet_id=${this.receiver}&type=${this.tab}`);
            const data = await response.json();
        ...
        async submitTransfer(){
          ...
          try {
            ...
            const response = await fetch('<?php echo e(route("admin.inventaris.transfer-gudang.store")); ?>', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
        ...
        exportPdf(){
          window.open('<?php echo e(route("admin.inventaris.transfer-gudang.export.pdf")); ?>', '_blank');
        },

        exportExcel(){
          window.open('<?php echo e(route("admin.inventaris.transfer-gudang.export.excel")); ?>', '_blank');
        },
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\inventaris\transfer-gudang\index.blade.php ENDPATH**/ ?>