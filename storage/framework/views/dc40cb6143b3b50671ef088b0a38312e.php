...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Detail Buku Akuntansi']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Detail Buku Akuntansi')]); ?>
    <div class="space-y-6">
    ...
    <script>
        function bookDetail() {
            return {
                ...
                async loadBook() {
                    try {
                        const url = '<?php echo e(route("finance.accounting-books.show", ["id" => ":id"])); ?>'.replace(':id', this.bookId);
                ...
                editBook(book) {
                    ...
                    const url = '<?php echo e(route("finance.buku.index")); ?>';
                ...
                async toggleBook(id, currentStatus) {
                    ...
                    try {
                        const url = '<?php echo e(route("finance.accounting-books.toggle", ["id" => ":id"])); ?>'.replace(':id', id);
                        const response = await fetch(url, {
                            method: 'POST',
                            headers: {
                ...
                async loadAccounts() {
                    try {
                        ...
                        const url = '<?php echo e(route("finance.chart-of-accounts.data")); ?>';
                ...
                async saveJournalEntry() {
                    ...
                    try {
                        ...
                        if (this.editingJournalEntry) {
                            url = '<?php echo e(route("finance.journal-entries.update", ["id" => ":id"])); ?>'.replace(':id', this.editingJournalEntry);
                            method = 'PUT';
                        } else {
                            url = '<?php echo e(route("finance.journal-entries.store")); ?>';
                            method = 'POST';
                        }
                ...
                viewJournalEntry(entry) {
                    try {
                        const url = '<?php echo e(route("finance.journal-entries.show", ["id" => ":id"])); ?>'.replace(':id', entry.id);
                ...
                editJournalEntry(entry) {
                    try {
                        const url = '<?php echo e(route("finance.journal-entries.edit", ["id" => ":id"])); ?>'.replace(':id', entry.id);
                ...
                async deleteJournalEntry(entry) {
                    ...
                    try {
                        const url = '<?php echo e(route("finance.journal-entries.delete", ["id" => ":id"])); ?>'.replace(':id', entry.id);
                        const response = await fetch(url, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                ...
                async closeBook(book) {
                    ...
                    try {
                        const url = '<?php echo e(route("finance.accounting-books.close", ["id" => ":id"])); ?>'.replace(':id', book.id);
                        const response = await fetch(url, {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                ...
                async generateBookReport(book) {
                    try {
                        ...
                        const url = '<?php echo e(route("finance.accounting-books.report", ["id" => ":id"])); ?>'.replace(':id', book.id);
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\finance\buku\detail.blade.php ENDPATH**/ ?>