    ...
    <div class="col-lg-12">
        <div class="box">
            <div class="box-body">
                <div class="row mb-3">
                    <div class="col-md-12">
                        <a href="<?php echo e(route('sales.invoice.history')); ?>" class="btn btn-success btn-sm" style="margin-right: 10px;">
                            <i class="fa fa-list"></i> History Invoice
                        </a>
                        <a href="<?php echo e(route('sales.coa.setting')); ?>" class="btn btn-warning btn-sm">
                            <i class="fa fa-cog"></i> Setting COA
                        </a>
                        <a href="<?php echo e(route('sales.ongkos-kirim.index')); ?>" class="btn btn-warning btn-sm">
                            <i class="fa fa-cog"></i> Setting Ongkir
                        </a>
                        <a href="<?php echo e(route('sales.customer-price.index')); ?>" class="btn btn-warning btn-sm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="date" name="tanggal" id="tanggal" class="form-control" value="<?php echo e(date('Y-m-d')); ?>" required>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function loadJournalPreview(total) {
            $.post('<?php echo e(route("sales.invoice.preview-journal-before-save")); ?>', {
        ...
        $('#invoice-form').submit(function(e) {
            ...
            $.ajax({
                url: '<?php echo e(route("sales.invoice.store")); ?>',
                type: 'POST',
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        window.location.href = '<?php echo e(route("sales.invoice.history")); ?>';
                    } else {
    ...
    function loadCustomers(search = '', page = 1) {
        ...
        $.get('<?php echo e(route("sales.get-customers")); ?>', { 
            search: search,
            page: page 
        }, function(response) {
            if (response.success) {
    ...
    // Reset form saat customer berubah
        ...
        function loadCustomerData(customerId, customerType) {
            ...
            $.get('<?php echo e(route("admin.penjualan.customer.detail", ["type" => ":type", "id" => ":id"])); ?>'.replace(':type', customerType).replace(':id', customerId), function(response) {
        ...
        async function loadCustomerTypePrices(idTipe) {
            try {
                const outletId = <?php echo e(auth()->user()->outlet_id ?? 1); ?>;
                const response = await fetch(`<?php echo e(route("admin.penjualan.pos.customer-type-prices")); ?>?id_tipe=${idTipe}&outlet_id=${outletId}`);
        ...
        function loadSemuaProduk(search = '') {
            const outletId = <?php echo e(auth()->user()->outlet_id ?? 1); ?>;
            $.get('<?php echo e(route("sales.get-produk-harga-normal")); ?>', { 
                search: search,
                outlet_id: outletId
            }, function(response) {
                if (response.success) {
...
<?php echo $__env->make('app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\hm\resources\views\sales_management\invoice\index.blade.php ENDPATH**/ ?>