...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Logistics - ' . $keberangkatan->keberangkatan_name]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Logistics - ' . $keberangkatan->keberangkatan_name)]); ?>
  <div x-data="logisticsManagement()" x-init="init()" class="space-y-4 self-start w-full">
    <!-- Header -->
    <div class="flex items-center justify-between">
      <div>
        <div class="flex items-center gap-2">
          <a href="<?php echo e(route('admin.inventaris.travel.keberangkatan.show', $keberangkatan->id)); ?>" class="p-2 hover:bg-slate-100 rounded-lg">
            <i class='bx bx-arrow-back text-xl'></i>
          </a>
          <div>
            <h1 class="text-2xl font-bold">Logistik & Perlengkapan</h1>
            <p class="text-slate-600 text-sm"><?php echo e($keberangkatan->keberangkatan_name); ?></p>
          </div>
        </div>
      </div>
      <div class="flex gap-2">
        <?php if($equipmentItems->isEmpty()): ?>
        ...
        <a href="<?php echo e(route('admin.inventaris.logistics.packing-list', $keberangkatan->id)); ?>" target="_blank"
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
      <div class="rounded-2xl border border-slate-200 bg-white shadow-card p-4">
        ...
        <div class="text-2xl font-bold"><?php echo e($stats['total_items']); ?></div>
      </div>
      <div class="rounded-2xl border border-slate-200 bg-white shadow-card p-4">
        <div class="text-sm text-slate-600">Belum Dipesan</div>
        <div class="text-2xl font-bold text-gray-600"><?php echo e($stats['not_ordered']); ?></div>
      </div>
      <div class="rounded-2xl border border-slate-200 bg-white shadow-card p-4">
        <div class="text-sm text-slate-600">Dipesan</div>
        <div class="text-2xl font-bold text-blue-600"><?php echo e($stats['ordered']); ?></div>
      </div>
      <div class="rounded-2xl border border-slate-200 bg-white shadow-card p-4">
        <div class="text-sm text-slate-600">Diterima</div>
        <div class="text-2xl font-bold text-indigo-600"><?php echo e($stats['received']); ?></div>
      </div>
      <div class="rounded-2xl border border-slate-200 bg-white shadow-card p-4">
        <div class="text-sm text-slate-600">Dikemas</div>
        <div class="text-2xl font-bold text-yellow-600"><?php echo e($stats['packed']); ?></div>
      </div>
      <div class="rounded-2xl border border-slate-200 bg-white shadow-card p-4">
      ...
      <div class="rounded-2xl border border-red-200 bg-red-50 shadow-card p-4">
        ...
        <div class="text-2xl font-bold text-red-600"><?php echo e($stats['approaching_deadline'] + $stats['overdue']); ?></div>
      </div>
    </div>
      ...
      <div class="p-5">
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody>
              <?php $__empty_1 = true; $__currentLoopData = $equipmentItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
              <tr class="border-b border-slate-100 hover:bg-slate-50">
                <td class="py-3 px-2 text-sm"><?php echo e($item->equipment_category ?? '-'); ?></td>
                <td class="py-3 px-2 text-sm font-medium"><?php echo e($item->equipment_name); ?></td>
                <td class="py-3 px-2 text-sm text-center"><?php echo e($item->quantity_needed); ?></td>
                <td class="py-3 px-2 text-sm text-center">
                  <span class="font-medium" :class="{'text-green-600': <?php echo e($item->quantity_received); ?> >= <?php echo e($item->quantity_needed); ?>}">
                    <?php echo e($item->quantity_received); ?>

                  </span>
                </td>
                <td class="py-3 px-2 text-center">
                  <span class="px-2 py-1 rounded-full text-xs font-medium bg-<?php echo e($item->getStatusBadgeColor()); ?>-100 text-<?php echo e($item->getStatusBadgeColor()); ?>-800">
                    <?php echo e($item->getStatusLabel()); ?>

                  </span>
                </td>
                <td class="py-3 px-2 text-sm"><?php echo e($item->supplier_name ?? '-'); ?></td>
                <td class="py-3 px-2 text-sm text-center">
                  <?php if($item->shipping_deadline): ?>
                    <div class="flex flex-col items-center">
                      <span class="<?php echo e($item->isDeadlineOverdue() ? 'text-red-600 font-medium' : ($item->isDeadlineApproaching() ? 'text-yellow-600 font-medium' : '')); ?>">
                ...
                <td class="py-3 px-2 text-center">
                  <div class="flex items-center justify-center gap-1">
                    <button x-on:click="openEditModal(<?php echo e($item->id); ?>)" 
                            class="p-1 hover:bg-slate-200 rounded">
                      <i class='bx bx-edit text-blue-600'></i>
                    </button>
                    <button x-on:click="openStatusModal(<?php echo e($item->id); ?>)" 
                            class="p-1 hover:bg-slate-200 rounded">
                      <i class='bx bx-refresh text-green-600'></i>
                    </button>
                    <button x-on:click="deleteItem(<?php echo e($item->id); ?>)" 
                            class="p-1 hover:bg-slate-200 rounded">
  ...
  <script>
    function logisticsManagement() {
      return {
        keberangkatanId: <?php echo e($keberangkatan->id); ?>,
        showModal: false,
        ...
        async saveEquipment() {
          ...
          try {
            const url = this.editMode 
              ? `<?php echo e(url('/admin/inventaris/travel/logistics')); ?>/${this.editingId}`
              : `<?php echo e(url('/admin/inventaris/travel/keberangkatan')); ?>/${this.keberangkatanId}/logistics`;
        ...
        async updateStatus() {
          this.saving = true;
          try {
            const response = await fetch(`<?php echo e(url('/admin/inventaris/travel/logistics')); ?>/${this.statusForm.id}/status`, {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        async deleteItem(id) {
          ...
          try {
            const response = await fetch(`<?php echo e(url('/admin/inventaris/travel/logistics')); ?>/${id}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
              }
        ...
        async initializeDefaults() {
          ...
          try {
            const response = await fetch(`<?php echo e(url('/admin/inventaris/travel/keberangkatan')); ?>/${this.keberangkatanId}/logistics/initialize`, {
              method: 'POST',
              headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
              }
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\travel\logistics\index.blade.php ENDPATH**/ ?>