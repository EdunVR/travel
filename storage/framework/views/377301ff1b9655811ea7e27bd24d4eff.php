...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Inventaris']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Inventaris')]); ?>
  <?php
    $user = auth()->user();
  ...
  <div x-data="inventarisDashboard(<?php echo e(json_encode($userOutlets->pluck('id_outlet')->toArray())); ?>, <?php echo e($defaultOutlet); ?>)" x-init="init()" class="space-y-6 overflow-x-hidden">
    ...
    <section x-show="!loading">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <a href="<?php echo e(route('admin.inventaris.transfer-gudang.index')); ?>"
           class="group rounded-2xl border border-slate-200 bg-white p-4 shadow-card hover:shadow-xl/10 hover:-translate-y-0.5 transition">
          <div class="flex items-start gap-3">
    ...
    <section x-show="!loading">
      <div class="flex items-center justify-between mb-2">
        ...
        <a href="<?php echo e(route('admin.inventaris.outlet.index')); ?>" class="text-sm text-primary-700 hover:underline">Kelola Outlet</a>
      </div>
  ...
  <script>
    function inventarisDashboard(availableOutlets, defaultOutlet){
      return {
        ...
        async loadStats(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.stats")); ?>' + this.getOutletParams());
        ...
        async loadOutletsSummary(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.outlets-summary")); ?>' + this.getOutletParams());
            const data = await response.json();
        ...
        async loadLowStockItems(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.low-stock-items")); ?>' + this.getOutletParams());
            const data = await response.json();
        ...
        async loadRecentActivities(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.recent-activities")); ?>' + this.getOutletParams());
            const data = await response.json();
        ...
        async performSearch(){
          ...
          try {
            const response = await fetch(`<?php echo e(route("admin.inventaris.search")); ?>?q=${encodeURIComponent(this.searchTerm)}` + (this.getOutletParams() ? '&' + this.getOutletParams().substring(1) : ''));
            const data = await response.json();
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\inventaris\index.blade.php ENDPATH**/ ?>