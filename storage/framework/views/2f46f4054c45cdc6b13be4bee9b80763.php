                                ...
                                <template x-for="cc in (package.hpp_calculation?.custom_components || [])" :key="cc.id">
                                    <div class="flex justify-between px-4 py-2.5 text-sm" x-show="(cc.value||0) > 0">
                                        <span class="text-slate-600">
                                            <span x-text="cc.label || 'Biaya Lainnya'"></span>
                                            <!-- Tampilkan qty dan satuan -->
                                            <span class="ml-1 text-xs text-slate-400"
                                                  x-text="'(qty: ' + (cc.qty || 1) + ' ' + (cc.satuan || 'unit') + ')'"></span>
                                            <!-- Info mata uang asli jika bukan IDR -->
                                            <span x-show="cc.currency && cc.currency !== 'IDR'"
                                                  class="ml-1 text-xs text-slate-400"
                                                  x-text="'— ' + (cc.value||0) + ' ' + (cc.currency||'IDR') + '/unit'"></span>
                                        </span>
                                        <div class="text-right">
                                            <!-- Unit price (per unit, IDR) -->
                                            <div class="font-semibold"
                                                 x-text="formatCurrency(
                                                     (cc.idr_value && cc.idr_value > 0)
                                                         ? (cc.idr_value / Math.max(1, cc.qty || 1))
                                                         : (cc.value || 0)
                                                 ) + ' /unit'"></div>
                                            <!-- Total (qty × unit price) -->
                                            <div class="text-xs text-green-700 font-medium"
                                                 x-text="'Total: ' + formatCurrency(cc.idr_value || (cc.value * (cc.qty || 1)))"></div>
                                        </div>
                                    </div>
                                </template>
    ...
    function packageDetail(packageId) {
        return {
            ...
            calculateHppPerPerson() {
                ...
                // Untuk custom components: gunakan idr_value (nilai setelah konversi kurs ke IDR)
                // Fallback ke value jika idr_value belum ada (data lama)
                if (this.package.hpp_calculation) {
                    ...
                    // Custom components: pakai idr_value (total sudah termasuk qty dan kurs)
                    // Dibagi qty agar jadi per-orang, sama seperti field standar
                    (this.package.hpp_calculation.custom_components || []).forEach(c => {
                        const idrVal = parseFloat(c.idr_value);
                        const rawVal = parseFloat(c.value) || 0;
                        const qty    = parseInt(c.qty) || 1;
                        if (!isNaN(idrVal) && idrVal > 0) {
                            // idr_value adalah total (value × qty × kurs), bagi qty untuk per-orang
                            total += idrVal / qty;
                        } else {
                            // Fallback: data lama tanpa idr_value
                            total += rawVal;
                        }
                    });
                    return total;
                        ...
                        if (Array.isArray(savedCustom) && savedCustom.length > 0) {
                            savedCustom.forEach(c => {
                                this.hppExtraComponents.push({
                                    ...
                                    label: c.label || '',  // Biarkan kosong, jangan override ke 'Komponen Lainnya'
                                    hint: '',
            ...
            async submitHppForm() {
                ...
                this.hppExtraComponents.forEach(c => {
                    if (knownKeys.includes(c.id)) {
                        ...
                        customComponents.push({
                            id: c.id,
                            label: c.label || '',  // Simpan label asli, jangan override ke 'Komponen Lainnya'
                            value: parseFloat(c.value) || 0,
                            idr_value: this.convertToIDR((parseFloat(c.value) || 0) * (parseInt(c.qty) || 1), c.currency || 'IDR'), // nilai sudah dikonversi ke IDR
                            payment_status: c.payment_status || 'hutang',
                            hutang_amount: parseFloat(c.hutang_amount) || 0,
                            currency: c.currency || 'IDR',
                ...
                // Gunakan idr_value (total dalam IDR) bukan value mentah
                const customTotal = customComponents.reduce((s, c) => s + (c.idr_value || c.value || 0), 0);
    ...
    function keberangkatanTab(packageId) {
        return {
            ...
            async regenerateRab() {
                ...
                try {
                    const url = `<?php echo e(url('')); ?>/admin/inventaris/travel/keberangkatan/${this.rabKb.id}/regenerate-rab`;
                    const res = await fetch(url, {
                        method: 'POST',
            ...
            async updateRabDetailItem(item, realisasi) {
                ...
                this.updatingRab = true;
                try {
                    const url = `<?php echo e(url('')); ?>/admin/inventaris/travel/keberangkatan/${this.rabKb.id}/rab-detail-update`;
                    const res = await fetch(url, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\travel\package\show.blade.php ENDPATH**/ ?>