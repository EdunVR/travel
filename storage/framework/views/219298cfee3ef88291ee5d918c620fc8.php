...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Inventaris / Produk']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Inventaris / Produk')]); ?>
  <div x-data="produkCrud()" x-init="init()" class="space-y-4 overflow-x-hidden">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.produk.create')): ?>
        ...
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.produk.export')): ?>
        ...
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.produk.import')): ?>
          ...
          <div class="group relative rounded-2xl border border-slate-200 bg-white shadow-card hover:shadow-[0_14px_40px_rgba(15,23,42,.10)] transition overflow-hidden">
            ...
            <div class="p-4">
              ...
              <div class="mt-3 flex gap-2">
                <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.produk.update')): ?>
                ...
                <?php endif; ?>
                <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.produk.hpp')): ?>
                ...
                <?php endif; ?>
                <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.produk.delete')): ?>
    ...
    <!-- Include HPP Modal -->
    <?php echo $__env->make('admin.inventaris.produk.hpp-modal', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
      ...
      return {
        ...
        async fetchData(){
          ...
          try {
            ...
            const response = await fetch(`<?php echo e(route('admin.inventaris.produk.data')); ?>?${params}`);
        ...
        async fetchOutlets(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.produk.outlets")); ?>');
            const data = await response.json();
        ...
        async fetchCategories(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.produk.categories")); ?>');
            const data = await response.json();
        ...
        async fetchUnits(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.produk.units")); ?>');
            const data = await response.json();
            this.units = data;
          } catch (error) {
        ...
        async openCreate(){ 
            try {
                ...
                const response = await fetch('<?php echo e(route("admin.inventaris.produk.generate-sku")); ?>');
                const data = await response.json();
        ...
        async openEdit(p){ 
          try {
            ...
            while (retryCount <= maxRetries) {
              try {
                response = await fetch(`<?php echo e(route('admin.inventaris.produk.show', '')); ?>/${productId}`);
        ...
        async submitAddStock() {
          ...
          try {
            ...
            const response = await fetch(`<?php echo e(url('admin/inventaris/produk')); ?>/${this.form.id}/add-stock`, {
              method: 'POST',
              headers: {
        ...
        async submitWizard(){
            ...
            try {
                ...
                const url = this.form.id 
                    ? `<?php echo e(route('admin.inventaris.produk.update', '')); ?>/${this.form.id}`
                    : '<?php echo e(route("admin.inventaris.produk.store")); ?>';
        ...
        async loadIdMappings() {
            ...
            try {
                const response = await fetch('<?php echo e(route("admin.inventaris.produk.id-mappings")); ?>');
                if (response.ok) {
                    const data = await response.json();
                    if (data.success) {
        ...
        async deleteNow(){
          ...
          this.deleting = true;
          try {
            const response = await fetch(`<?php echo e(route('admin.inventaris.produk.destroy', '')); ?>/${this.toDelete.id}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                'Content-Type': 'application/json'
        ...
        async removeImage(index) {
          ...
          try {
            ...
            if (this.form.id && this.form.images[index] && this.form.images[index].id) {
              ...
              const url = '<?php echo e(route("admin.inventaris.produk.remove-image", ":productId")); ?>'.replace(':productId', productId);
        ...
        async setPrimaryImage(index) {
          try {
            ...
            if (this.form.id && this.form.images[index] && this.form.images[index].id) {
              ...
              // Use route helper to generate correct URL
              const url = '<?php echo e(route("admin.inventaris.produk.set-primary-image", ":productId")); ?>'.replace(':productId', productId);
              console.log('Fetching URL:', url);
        ...
        exportPdf(){
          ...
          window.open(`<?php echo e(route('admin.inventaris.produk.export.pdf')); ?>?${params}`, '_blank');
        },
        ...
        exportExcel(){
          ...
          window.open(`<?php echo e(route('admin.inventaris.produk.export.excel')); ?>?${params}`, '_blank');
        },
        ...
        async importExcel(event){
          ...
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.produk.import.excel")); ?>', {
              method: 'POST',
              headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
              },
        ...
        downloadTemplate(){
          window.open('<?php echo e(route("admin.inventaris.produk.download-template")); ?>', '_blank');
        },
        ...
        async fetchHppData() {
          ...
          try {
            const response = await fetch(`<?php echo e(url('admin/inventaris/produk')); ?>/${productId}/hpp-data`);
            const data = await response.json();
        ...
        async submitHppForm() {
          ...
          try {
            ...
            const response = await fetch(`<?php echo e(url('admin/inventaris/produk')); ?>/${productId}/hpp`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
        ...
        async submitEditHppForm() {
          ...
          try {
            ...
            const response = await fetch(`<?php echo e(url('admin/inventaris/produk')); ?>/${productId}/hpp/${this.editHppForm.id}`, {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
        ...
        async deleteHppNow() {
          ...
          try {
            const response = await fetch(`<?php echo e(url('admin/inventaris/produk')); ?>/${this.selectedProduct.id}/hpp/${this.hppToDelete.id}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                'Accept': 'application/json'
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\inventaris\produk\index.blade.php ENDPATH**/ ?>