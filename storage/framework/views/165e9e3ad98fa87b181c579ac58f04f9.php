...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Tipe & Diskon Customer']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Tipe & Diskon Customer')]); ?>
  ...
  <div x-data="customerTypeManagement()" x-init="init()" class="space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'crm.tipe.create')): ?>
    ...
    function customerTypeManagement() {
      return {
        ...
        loadData() {
          ...
          fetch(`<?php echo e(route("admin.crm.tipe.data")); ?>?${params}`)
            .then(res => res.json())
            .then(data => {
              if (data.data) {
        ...
        loadStatistics() {
          ...
          fetch(`<?php echo e(route('admin.crm.tipe.statistics')); ?>?${params}`)
            .then(res => res.json())
            .then(data => {
              if (data.success) {
        ...
        loadTypeData(id) {
          fetch(`<?php echo e(url('admin/crm/tipe')); ?>/${id}`)
            .then(res => res.json())
            .then(data => {
              if (data.success) {
                this.formData = {
        ...
        async submitForm() {
          ...
          try {
            const url = this.editMode 
              ? `<?php echo e(url('admin/crm/tipe')); ?>/${this.editId}`
              : '<?php echo e(route("admin.crm.tipe.store")); ?>';
        ...
        async deleteType(id) {
          ...
          try {
            const response = await axios.delete(`<?php echo e(url('admin/crm/tipe')); ?>/${id}`);
        ...
        loadTypeProducts(typeId) {
          fetch(`<?php echo e(url('admin/crm/tipe')); ?>/${typeId}/products`)
            .then(res => res.json())
            .then(data => {
              if (data.success) {
        ...
        searchProducts() {
          ...
          fetch(`<?php echo e(route('admin.crm.tipe.search-products')); ?>?${params}`)
            .then(res => res.json())
            .then(data => {
              if (data.success) {
        ...
        async submitAddProduct() {
          ...
          try {
            const response = await axios.post(
              `<?php echo e(url('admin/crm/tipe')); ?>/${this.selectedTypeId}/products`,
              { 
        ...
        async updateProductDiscount(produkTipeId, diskon, hargaJual) {
          try {
            const response = await axios.put(
              `<?php echo e(url('admin/crm/tipe/products')); ?>/${produkTipeId}`,
              { 
                diskon: diskon || 0,
        ...
        async removeProductFromType(produkTipeId) {
          ...
          try {
            const response = await axios.delete(`<?php echo e(url('admin/crm/tipe/products')); ?>/${produkTipeId}`);
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\crm\tipe\index.blade.php ENDPATH**/ ?>