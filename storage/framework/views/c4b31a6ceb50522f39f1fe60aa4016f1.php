...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Penyesuaian Info Paket']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Penyesuaian Info Paket')]); ?>
    <!-- Flatpickr CSS & JS -->
    ...
    <div x-data="infoPaketForm()" x-init="init()" class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <a href="<?php echo e(route('admin.inventaris.travel.package.detail', $package->id)); ?>" class="inline-flex items-center gap-2 text-blue-600 hover:text-blue-700 mb-2">
                    <i class="bx bx-arrow-back"></i> Kembali ke Detail Paket
                </a>
                <h1 class="text-2xl font-bold">Penyesuaian Info Paket</h1>
                <p class="text-gray-600"><?php echo e($package->package_name); ?> — <?php echo e($keberangkatan->keberangkatan_name); ?></p>
            </div>
    ...
    function infoPaketForm() {
        return {
            ...
            dateForm: {
                departure_datetime: '<?php echo e($package->departure_datetime ? $package->departure_datetime->format("d/m/Y H:i") : ($package->departure_date ? \Carbon\Carbon::parse($package->departure_date)->format("d/m/Y") . " 00:00" : "")); ?>',
                return_datetime: '<?php echo e($package->return_datetime ? $package->return_datetime->format("d/m/Y H:i") : ($package->return_date ? \Carbon\Carbon::parse($package->return_date)->format("d/m/Y") . " 00:00" : "")); ?>'
            },
            ...
            async init() {
                ...
                try {
                    const res = await fetch(`<?php echo e(route('admin.inventaris.travel.package.info-paket.data', ['id' => $package->id, 'keberangkatanId' => $keberangkatan->id])); ?>`);
            ...
            async saveDates() {
                ...
                try {
                    const res = await fetch(`<?php echo e(route('admin.inventaris.travel.package.update-dates', ['id' => $package->id, 'keberangkatanId' => $keberangkatan->id])); ?>`, {
                        method: 'POST',
                        headers: {
            ...
            async saveData() {
                this.saving = true;
                try {
                    const res = await fetch(`<?php echo e(route('admin.inventaris.travel.package.info-paket.save', ['id' => $package->id, 'keberangkatanId' => $keberangkatan->id])); ?>`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            async saveAndDownload() {
                await this.saveData();
                // Open PDF in new tab
                window.open(`<?php echo e(route('admin.inventaris.travel.package.info-paket.pdf', ['id' => $package->id, 'keberangkatanId' => $keberangkatan->id])); ?>`, '_blank');
            }
        };
    }
    </script>
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views/admin/travel/package/info-paket-form.blade.php ENDPATH**/ ?>