...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Setting Fee Jenjang Mitra']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Setting Fee Jenjang Mitra')]); ?>
...
<div class="space-y-6">
    ...
    <form action="<?php echo e(route('admin.inventaris.affiliate.hierarchy.settings.save')); ?>" method="POST">
        <?php echo csrf_field(); ?>
        <div class="bg-white rounded-xl border border-slate-200 shadow-card overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <?php $isFlat = ($setting->fee_type ?? 'percentage') === 'flat'; ?>
                        <tr class="hover:bg-slate-50">
                            <input type="hidden" name="settings[<?php echo e($i); ?>][from_level]" value="<?php echo e($setting->from_level); ?>">
                            <input type="hidden" name="settings[<?php echo e($i); ?>][to_level]" value="<?php echo e($setting->to_level); ?>">
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium
                                    <?php echo e($setting->from_level === 'hm-seller' ? 'bg-blue-100 text-blue-700' :
                                    ...
                                    <?php echo e($setting->from_level_label); ?>

                                </span>
                            </td>
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium
                                    <?php echo e($setting->to_level === 'hm-partner' ? 'bg-green-100 text-green-700' :
                                       ($setting->to_level === 'hm-leader' ? 'bg-indigo-100 text-indigo-700' : 'bg-purple-100 text-purple-700')); ?>">
                                    <i class="fas fa-arrow-up text-xs"></i>
                                    <?php echo e($setting->to_level_label); ?>

                                </span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <select name="settings[<?php echo e($i); ?>][fee_type]"
                                        onchange="toggleFeeInput(this, <?php echo e($i); ?>)"
                                        class="h-8 px-2 rounded-lg border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-primary-300">
                                    <option value="percentage" <?php echo e(!$isFlat ? 'selected' : ''); ?>>% Persen</option>
                                    <option value="flat"       <?php echo e($isFlat  ? 'selected' : ''); ?>>Rp Nominal</option>
                                </select>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <input type="number"
                                           name="settings[<?php echo e($i); ?>][fee_value]"
                                           id="fee-val-<?php echo e($i); ?>"
                                           value="<?php echo e($setting->fee_value ?? $setting->percentage); ?>"
                                           min="0"
                                           max="<?php echo e($isFlat ? '' : '100'); ?>"
                                           step="<?php echo e($isFlat ? '1000' : '0.5'); ?>"
                                    ...
                                    <span id="fee-unit-<?php echo e($i); ?>" class="text-slate-400 text-xs"><?php echo e($isFlat ? 'Rp' : '%'); ?></span>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <input type="checkbox" name="settings[<?php echo e($i); ?>][is_active]" value="1"
                                       <?php echo e($setting->is_active ? 'checked' : ''); ?>

                                       class="w-4 h-4 rounded text-primary-600">
                            </td>
                            <td class="px-4 py-3">
                                <input type="text" name="settings[<?php echo e($i); ?>][notes]"
                                       value="<?php echo e($setting->notes); ?>"
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\affiliate\hierarchy-settings.blade.php ENDPATH**/ ?>