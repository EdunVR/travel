    ...
    <div class="card border-0 shadow-sm">
        ...
        <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            ...
                            <div class="input-group">
                                <input type="text" name="search" id="search" class="form-control" value="<?php echo e(request('search')); ?>" placeholder="No. Jurnal/No. Bukti/Deskripsi">
            ...
            <div class="alert alert-info d-flex justify-content-between">
                <strong>Total Debit: <?php echo e(number_format($totals->total_debit ?? 0, 2)); ?></strong>
                <strong>Total Kredit: <?php echo e(number_format($totals->total_credit ?? 0, 2)); ?></strong>
                <strong>Selisih: <?php echo e(number_format(($totals->total_debit ?? 0) - ($totals->total_credit ?? 0), 2)); ?></strong>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <?php $__currentLoopData = $journals; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $journal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr>
                            <td>
                                <input type="checkbox" class="journal-checkbox" value="<?php echo e($journal->id); ?>">
                            </td>
                            <td><?php echo e($loop->iteration + ($journals->currentPage() - 1) * $journals->perPage()); ?></td>
                            <td><?php echo e($journal->journal_number); ?></td>
                            <td><?php echo e($journal->transaction_date->format('d/m/Y')); ?></td>
                            <td><?php echo e($journal->reference_number); ?></td>
                            ...
                            <td style="text-align: left;">
                                ...
                                <div>
                                    <?php echo e($entry->account_code); ?> - <?php echo e($accountNames[$entry->account_code] ?? ''); ?>

                                    <?php if($entry->subClass): ?>
                                    <small>(<?php echo e($entry->subClass->name); ?>)</small>
                                    <?php endif; ?>
                                </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </td>
                            <td style="text-align: right;">
                                <?php $__currentLoopData = $journal->entries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $entry): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div><?php echo e(number_format($entry->debit, 2)); ?></div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </td>
                            <td style="text-align: right;">
                                <?php $__currentLoopData = $journal->entries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $entry): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div><?php echo e(number_format($entry->credit, 2)); ?></div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </td>
                            <td><?php echo e($journal->creator->name); ?></td>
                            <td>
                                <?php if($journal->is_validated): ?>
                                    <span class="badge bg-success">Valid</span>
                                    <small class="d-block text-muted">
                                        <?php echo e($journal->validated_at->format('d/m/Y H:i')); ?>
                            ...
                            <div class="btn-group btn-group-sm" role="group">
                                <?php if(!$journal->is_validated): ?>
                                    <button class="btn btn-success validate-btn" data-id="<?php echo e($journal->id); ?>" title="Validasi">
                                        <i data-feather="check"></i>
                                    </button>
                                    <button class="btn btn-primary edit-btn" data-id="<?php echo e($journal->id); ?>" title="Edit">
                                        <i data-feather="edit"></i>
                                    </button>
                                <?php endif; ?>
                                <button class="btn btn-danger delete-btn" data-id="<?php echo e($journal->id); ?>" title="Hapus">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            ...
                            <select name="accounting_book_id" id="accounting_book_id" class="form-select" required>
                                ...
                                <?php $__currentLoopData = $booksActive; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $book): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($book->id); ?>"><?php echo e($book->name); ?> (<?php echo e($book->start_date->format('Y')); ?>)</option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
...
$(document).ready(function() {
    ...
    function openCreateModal() {
        ...
        $.get("<?php echo e(route('financial.journal.generate-reference')); ?>", function(response) {
    ...
    function initAutocomplete() {
        $('.account-code-autocomplete').autocomplete({
            source: function(request, response) {
                $.get("<?php echo e(route('financial.journal.search-accounts')); ?>", {
                    term: request.term
                }, function(data) {
                    response(data);
                });
    ...
    function updateBalanceDisplay(row, accountCode) {
        ...
        $.ajax({
            url: "<?php echo e(route('financial.journal.account.balance')); ?>",
            type: 'GET',
            data: {
            ...
            <tr class="entry-row" data-index="${index}">
                ...
                <td>
                    <select name="entries[${index}][sub_class_id]" class="form-select">
                        ...
                        <?php $__currentLoopData = $subClasses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($sc->id); ?>" ${entryData?.sub_class_id == <?php echo e($sc->id); ?> ? 'selected' : ''}>
                                <?php echo e($sc->code); ?> - <?php echo e($sc->name); ?>
    ...
    $('#validateSelectedBtn').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?php echo e(route("financial.journal.validate-selected")); ?>',
                    type: 'POST',
                    data: {
                        _token: '<?php echo e(csrf_token()); ?>',
                        ids: selectedIds
    ...
    $('#deleteSelectedBtn').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?php echo e(route("financial.journal.delete-selected")); ?>',
                    type: 'POST',
                    data: {
                        _token: '<?php echo e(csrf_token()); ?>',
                        ids: selectedIds
...
<?php echo $__env->make('app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\hm\resources\views\financial\journal\index.blade.php ENDPATH**/ ?>