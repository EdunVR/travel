...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Inventaris / Satuan']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Inventaris / Satuan')]); ?>
  <div x-data="satuanCrud()" x-init="init()" class="space-y-4 overflow-x-hidden">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.satuan.create')): ?>
        ...
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.satuan.export')): ?>
        ...
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.satuan.import')): ?>
        ...
        <template x-for="u in satuan" :key="u.id">
          <div class="rounded-2xl border border-slate-200 bg-white shadow-card hover:shadow-[0_14px_40px_rgba(15,23,42,.10)] transition p-4">
            ...
            <div class="mt-3 flex gap-2">
              <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.satuan.edit')): ?>
              ...
              <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.satuan.delete')): ?>
      ...
      return {
        ...
        async fetchData(){
          ...
          try {
            ...
            const response = await fetch(`<?php echo e(route('admin.inventaris.satuan.data')); ?>?${params}`);
        ...
        async fetchSatuanUtama(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.satuan.satuan-utama")); ?>');
            const data = await response.json();
        ...
        async openCreate(){ 
          try {
            ...
            const [kodeResponse, satuanUtamaResponse] = await Promise.all([
              fetch('<?php echo e(route("admin.inventaris.satuan.generate-kode")); ?>'),
              fetch('<?php echo e(route("admin.inventaris.satuan.satuan-utama")); ?>')
            ]);
        ...
        async openEdit(u){ 
          try {
            ...
            const [detailResponse, satuanUtamaResponse] = await Promise.all([
              fetch(`<?php echo e(route('admin.inventaris.satuan.show', '')); ?>/${u.id}`),
              fetch('<?php echo e(route("admin.inventaris.satuan.satuan-utama")); ?>')
            ]);
        ...
        async submitForm(){
          ...
          try {
            const url = this.form.id 
              ? `<?php echo e(route('admin.inventaris.satuan.update', '')); ?>/${this.form.id}`
              : '<?php echo e(route("admin.inventaris.satuan.store")); ?>';
        ...
        async deleteNow(){
          ...
          this.deleting = true;
          try {
            const response = await fetch(`<?php echo e(route('admin.inventaris.satuan.destroy', '')); ?>/${this.toDelete.id}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                'Content-Type': 'application/json'
        ...
        exportPdf(){
          ...
          window.open(`<?php echo e(route('admin.inventaris.satuan.export.pdf')); ?>?${params}`, '_blank');
        },
        ...
        exportExcel(){
          ...
          window.open(`<?php echo e(route('admin.inventaris.satuan.export.excel')); ?>?${params}`, '_blank');
        },
        ...
        async importExcel(event){
          ...
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.satuan.import.excel")); ?>', {
              method: 'POST',
              headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
              },
        ...
        downloadTemplate(){
          window.open('<?php echo e(route("admin.inventaris.satuan.download-template")); ?>', '_blank');
        },
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\inventaris\satuan\index.blade.php ENDPATH**/ ?>