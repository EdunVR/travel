...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'SDM / Absensi']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('SDM / Absensi')]); ?>
  <div x-data="attendanceCrud()" x-init="init()" class="space-y-4 overflow-x-hidden">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'hrm.absensi.create')): ?>
      ...
      <div class="p-4">
        <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
          ...
          <div class="md:col-span-3 flex items-end gap-2">
            ...
            <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'hrm.absensi.export')): ?>
            <button 
                ...
                <tr class="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td class="px-4 py-3">
                    <div class="flex gap-2 justify-center">
                      <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'hrm.absensi.edit')): ?>
                      <button 
                      ...
                      <?php endif; ?>
                      <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'hrm.absensi.delete')): ?>
                      <button 
    ...
    function attendanceCrud() {
      ...
      return {
        ...
        async fetchDailyData() {
          ...
          const response = await fetch(`<?php echo e(route('sdm.attendance.daily.table')); ?>?${params}`);
        ...
        async fetchMonthlyData() {
          ...
          const response = await fetch(`<?php echo e(route('sdm.attendance.monthly.table')); ?>?${params}`);
          const data = await response.json();
        ...
        async fetchEmployees() {
          try {
            ...
            const response = await fetch(`<?php echo e(route("sdm.attendance.employees")); ?>?${params}`);
            const data = await response.json();
            this.employees = data;
        ...
        async fetchStatistics() {
          try {
            ...
            const response = await fetch(`<?php echo e(route('sdm.attendance.statistics')); ?>?${params}`);
            const data = await response.json();
        ...
        async openEdit(id, employeeId = null, date = null) {
          try {
            ...
            const response = await fetch(`<?php echo e(route('sdm.attendance.show', '')); ?>/${id}`);
            const data = await response.json();
        ...
        async submitForm() {
          ...
          try {
            const url = this.form.id 
              ? `<?php echo e(route('sdm.attendance.update', '')); ?>/${this.form.id}`
              : '<?php echo e(route("sdm.attendance.store")); ?>';
        ...
        async submitWorkHours() {
          ...
          try {
            const response = await fetch('<?php echo e(route("sdm.attendance.set.work.hours")); ?>', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
        ...
        async loadTimeSettings() {
          ...
          try {
            const response = await fetch('<?php echo e(route("sdm.attendance.time.settings")); ?>');
            const result = await response.json();
        ...
        async saveTimeSettings() {
          ...
          try {
            ...
            const response = await fetch('<?php echo e(route("sdm.attendance.time.settings.update")); ?>', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
        ...
        async testTimePeriod() {
          ...
          try {
            const response = await fetch('<?php echo e(route("sdm.attendance.test.time.period")); ?>', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
        ...
        async deleteNow() {
          ...
          this.deleting = true;
          try {
            const response = await fetch(`<?php echo e(route('sdm.attendance.destroy', '')); ?>/${this.deleteId}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                'Content-Type': 'application/json'
        ...
        exportPdf() {
          ...
          if (this.currentTab === 'daily') {
            params.append('date', this.filterDate);
            window.open(`<?php echo e(route('sdm.attendance.export.daily.pdf')); ?>?${params}`, '_blank');
          } else {
            params.append('month', this.filterMonth);
            params.append('year', this.filterYear);
            window.open(`<?php echo e(route('sdm.attendance.export.monthly.pdf')); ?>?${params}`, '_blank');
          }
        },
        ...
        exportExcel() {
          ...
          window.open(`<?php echo e(route('sdm.attendance.export.excel')); ?>?${params}`, '_blank');
        },
        ...
        showPhotoModal(photoPath, title) {
          ...
          this.photoModalSrc = '<?php echo e(asset("storage")); ?>/' + photoPath;
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views/admin/sdm/attendance/index.blade.php ENDPATH**/ ?>