...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Inventaris / Kategori Umum']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Inventaris / Kategori Umum')]); ?>
  <div x-data="kategoriCrud()" x-init="init()" class="space-y-4 overflow-x-hidden">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.kategori.create')): ?>
        ...
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.kategori.export')): ?>
        ...
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.kategori.import')): ?>
        ...
        <template x-for="k in kategori" :key="k.id">
          <div class="rounded-2xl border border-slate-200 bg-white shadow-card hover:shadow-[0_14px_40px_rgba(15,23,42,.10)] transition p-4">
            ...
            <div class="mt-3 flex gap-2">
              <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.kategori.edit')): ?>
              ...
              <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'inventaris.kategori.delete')): ?>
      ...
      return {
        ...
        async fetchData(){
          ...
          try {
            ...
            const response = await fetch(`<?php echo e(route('admin.inventaris.kategori.data')); ?>?${params}`);
        ...
        async fetchOutlets(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.kategori.outlets")); ?>');
            const data = await response.json();
        ...
        async fetchGroups(){
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.kategori.groups")); ?>');
            const data = await response.json();
        ...
        async openCreate(){ 
          try {
            ModalLoader.show();
            const response = await fetch('<?php echo e(route("admin.inventaris.kategori.generate-kode")); ?>');
            const data = await response.json();
        ...
        async openEdit(k){ 
          ...
          try {
              const response = await fetch(`<?php echo e(route('admin.inventaris.kategori.show', '')); ?>/${k.id}`);
        ...
        async submitForm(){
          ...
          try {
            const url = this.form.id 
              ? `<?php echo e(route('admin.inventaris.kategori.update', '')); ?>/${this.form.id}`
              : '<?php echo e(route("admin.inventaris.kategori.store")); ?>';
        ...
        async deleteNow(){
          ...
          this.deleting = true;
          try {
            const response = await fetch(`<?php echo e(route('admin.inventaris.kategori.destroy', '')); ?>/${this.toDelete.id}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                'Content-Type': 'application/json'
        ...
        exportPdf(){
          ...
          window.open(`<?php echo e(route('admin.inventaris.kategori.export.pdf')); ?>?${params}`, '_blank');
        },
        ...
        exportExcel(){
          ...
          window.open(`<?php echo e(route('admin.inventaris.kategori.export.excel')); ?>?${params}`, '_blank');
        },
        ...
        async importExcel(event){
          ...
          try {
            const response = await fetch('<?php echo e(route("admin.inventaris.kategori.import.excel")); ?>', {
              method: 'POST',
              headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
              },
        ...
        downloadTemplate(){
          window.open('<?php echo e(route("admin.inventaris.kategori.download-template")); ?>', '_blank');
        },
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\inventaris\kategori\index.blade.php ENDPATH**/ ?>