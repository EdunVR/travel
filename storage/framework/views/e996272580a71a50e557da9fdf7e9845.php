...
#invoice-modal .modal-content { max-height: calc(100vh - 2rem); }
  ...
  <script>
    ...
    const BOOKING_BASE_URL = '<?php echo e(url("admin/inventaris/travel/booking")); ?>';
...
7. Jika terdapat Force Majure maka tidak dapat dibebankan kepada travel.</textarea>
          ...
          <div id="adminQrisAmountForm">
            <div class="mb-3">
              ...
              <input type="number" class="form-control form-control-lg text-center" 
                     id="adminQrisAmount" 
                     value="<?php echo e($booking->remaining_amount ?? ($booking->total_price - ($booking->paid_amount ?? 0))); ?>"
                     min="1000" step="1000">
              <small class="text-muted">Sisa tagihan: Rp <?php echo e(number_format($booking->remaining_amount ?? ($booking->total_price - ($booking->paid_amount ?? 0)), 0, ',', '.')); ?></small>
            </div>
  ...
  function adminGenerateQris() {
      ...
      fetch('<?php echo e(route("admin.inventaris.payment.qris.generate", $booking->id)); ?>', {
          method: 'POST',
          headers: {
  ...
  function showAdminQrisCode(data) {
      ...
      adminQrisCheckInterval = setInterval(function() {
          fetch('<?php echo e(url("/qris")); ?>/' + data.trx_number + '/check-status', { headers: {'Accept':'application/json'} })
          .then(r => r.json())
          .then(res => {
              if (res.paid) {
                  clearInterval(adminQrisCheckInterval);
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\travel\booking\show.blade.php ENDPATH**/ ?>