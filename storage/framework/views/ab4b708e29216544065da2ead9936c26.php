        ...
        <div class="box">
            <div class="box-body">
                <div class="mobile-action-buttons" style="display: none; margin-bottom: 15px;">
                    <a href="<?php echo e(route('service.invoice.history')); ?>" class="btn btn-info btn-block btn-sm" style="margin-bottom: 10px;">
                        <i class="fa fa-arrow-left"></i> Kembali ke History
                    </a>
                </div>
                
                <a href="<?php echo e(route('service.invoice.history')); ?>" class="btn btn-success btn-sm desktop-btn" style="margin-right: 10px;">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="date" name="tanggal" id="tanggal" class="form-control" value="<?php echo e(date('Y-m-d')); ?>" required readonly>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="tanggal_mulai_service">Tanggal Mulai Service</label>
                                <input type="date" name="tanggal_mulai_service" id="tanggal_mulai_service" class="form-control" value="<?php echo e(date('Y-m-d')); ?>" required>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="tanggal_selesai_service">Tanggal Selesai Service</label>
                                <input type="date" name="tanggal_selesai_service" id="tanggal_selesai_service" class="form-control" value="<?php echo e(date('Y-m-d')); ?>" required>
                            </div>
                        </div>
                    </div>
...
function parseNumber(formattedNumber) {
    ...
    $(document).ready(function() {
        ...
        $('#id_member').change(function() {
            ...
            if (memberId) {
                $.get('<?php echo e(route("service.get-mesin-customer-grouped", "")); ?>/' + memberId, function(mesinCustomers) {
                    $('#id_mesin_customer').empty().append('<option value="">Pilih Mesin Customer</option>');
        ...
        function submitInvoiceForm(formData) {
            ...
            $.ajax({
                url: '<?php echo e(route("service.invoice.store")); ?>',
                type: 'POST',
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                window.location.href = '<?php echo e(route("service.invoice.history")); ?>';
                            }
                        });
                    } else {
                        Swal.fire({
        ...
        $('#preview-invoice').click(function() {
            ...
            if (mesinCustomerId && jenisService) {
                ...
                const previewUrl = '<?php echo e(route("service.invoice.preview.temp")); ?>?data=' + encodedData + '&preview=true&timestamp=' + new Date().getTime();
...
function selectSparepart(sparepartId) {
    $.get(`<?php echo e(url('sparepart')); ?>/${sparepartId}/detail`, function(sparepart) {
...
<?php echo $__env->make('app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\hm\resources\views\service_management\invoice\index.blade.php ENDPATH**/ ?>