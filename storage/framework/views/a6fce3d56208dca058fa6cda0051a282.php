...
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('SDM / Manajemen Kinerja')]); ?>
  ...
  function kinerjaCrud() {
    return {
      ...
      // Form
      form: {
        outlet_id: <?php echo e(auth()->user()->outlets()->first()->id_outlet ?? 'null'); ?>,
        recruitment_id: '',
      ...
      async fetchData() {
        ...
        try {
          ...
          const response = await fetch(`<?php echo e(route('sdm.kinerja.data')); ?>?${params}`);
      ...
      async fetchStatistics() {
        try {
          ...
          const response = await fetch(`<?php echo e(route('sdm.kinerja.statistics')); ?>?${params}`);
          const data = await response.json();
      ...
      async fetchEmployees() {
        try {
          const response = await fetch(`<?php echo e(route('sdm.kinerja.employees')); ?>`);
          const data = await response.json();
      ...
      async openEdit(id) {
        ...
        try {
          const response = await fetch(`<?php echo e(route('sdm.kinerja.index')); ?>/${id}`);
          const data = await response.json();
      ...
      async saveData() {
        ...
        try {
          const url = this.editId 
            ? `<?php echo e(route('sdm.kinerja.index')); ?>/${this.editId}`
      ...
      exportPdf() {
        ...
        window.open(`<?php echo e(route('sdm.kinerja.export.pdf')); ?>?${params}`, '_blank');
      },
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\sdm\kinerja\index-new.blade.php ENDPATH**/ ?>