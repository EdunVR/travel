...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Investor / Bagi Hasil']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Investor / Bagi Hasil')]); ?>
  <div x-data="investorBagiHasil()" x-init="init()" class="space-y-4 overflow-x-hidden">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('investor.bagi-hasil.create')): ?>
        ...
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('investor.bagi-hasil.export')): ?>
        ...
        <template x-for="item in bagiHasil" :key="item.id">
          <div class="rounded-2xl border border-slate-200 bg-white shadow-card hover:shadow-[0_14px_40px_rgba(15,23,42,.10)] transition p-4">
            ...
            <div class="mt-3 flex gap-2">
              ...
              <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('investor.bagi-hasil.update')): ?>
              <button x-on:click="openEdit(item)" class="flex-1 rounded-lg border border-slate-200 px-3 py-2 hover:bg-slate-50 text-sm">
                <i class='bx bx-edit-alt'></i> Edit
              </button>
              <?php endif; ?>
              <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('investor.bagi-hasil.delete')): ?>
      ...
      return {
        ...
        async fetchData(){
          ...
          try {
            ...
            const response = await fetch(`<?php echo e(route('admin.investor.bagi-hasil.index')); ?>?${params}`);
        ...
        async submitForm(){
          ...
          try {
            const url = this.form.id 
              ? `<?php echo e(route('admin.investor.bagi-hasil.index')); ?>/${this.form.id}`
              : '<?php echo e(route("admin.investor.bagi-hasil.store")); ?>';
        ...
        async deleteNow(){
          ...
          this.deleting = true;
          try {
            const response = await fetch(`<?php echo e(route('admin.investor.bagi-hasil.index')); ?>/${this.toDelete.id}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                'Content-Type': 'application/json'
        ...
        exportPdf(){
          ...
          window.open(`<?php echo e(route('admin.investor.bagi-hasil.export')); ?>?${params}&format=pdf`, '_blank');
        },
        ...
        exportExcel(){
          ...
          window.open(`<?php echo e(route('admin.investor.bagi-hasil.export')); ?>?${params}&format=excel`, '_blank');
        },
...
<?php endif; ?>
                ...
                async markAsPaid(id) {
                    ...
                    try {
                        const response = await fetch(`<?php echo e(route("admin.investor.bagi-hasil.index")); ?>/${id}/mark-as-paid`, {
                            method: 'POST',
                            headers: {
                ...
                async deleteProfitShare(id) {
                    ...
                    try {
                        const response = await fetch(`<?php echo e(route("admin.investor.bagi-hasil.index")); ?>/${id}`, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                            }
...
<?php endif; ?><?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\investor\bagi-hasil\index.blade.php ENDPATH**/ ?>