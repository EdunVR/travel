...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Neraca Saldo (Trial Balance)']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Neraca Saldo (Trial Balance)')]); ?>
    ...
    <div class="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl shadow-lg p-6 text-white">
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
          <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'finance.neraca-saldo.export')): ?>
  ...
  <script>
    function neracaSaldoManagement() {
      return {
        ...
        async loadBooks() {
          try {
            const response = await fetch('<?php echo e(route("finance.accounting-books.data")); ?>');
            const result = await response.json();
            if (result.success) {
        ...
        async loadData() {
          ...
          try {
            ...
            const response = await fetch(`<?php echo e(route('finance.trial-balance.data')); ?>?${params}`);
            const result = await response.json();
        ...
        async viewAccountDetails(account) {
          ...
          try {
            ...
            const response = await fetch(`<?php echo e(route('finance.general-ledger.account-details')); ?>?${params}`);
            const result = await response.json();
        ...
        exportPDF() {
          const params = new URLSearchParams(this.filters);
          window.open(`<?php echo e(route('finance.trial-balance.export.pdf')); ?>?${params}`, '_blank');
        },
        
        // Export to Excel
        exportExcel() {
          const params = new URLSearchParams(this.filters);
          window.location.href = `<?php echo e(route('finance.trial-balance.export.excel')); ?>?${params}`;
        },
