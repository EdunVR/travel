...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Aktiva Tetap']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Aktiva Tetap')]); ?>
  <div x-data="fixedAssetsManagement()" x-init="init()" class="space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'finance.aktiva.create')): ?>
        ...
        <div class="flex rounded-xl border border-slate-200 bg-white overflow-hidden">
          <button @click="switchViewMode('grid')" 
                  :class="viewMode === 'grid' ? 'bg-blue-500 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'"
                  class="px-3 py-2 flex items-center gap-2 transition-colors">
            <i class='bx bx-grid-alt text-sm'></i> Grid
          </button>
          <button @click="switchViewMode('table')" 
                  :class="viewMode === 'table' ? 'bg-blue-500 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'"
                  class="px-3 py-2 flex items-center gap-2 transition-colors border-l border-slate-200">
            <i class='bx bx-table text-sm'></i> Tabel
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-card">
      <div class="flex flex-wrap gap-3 items-center">
        ...
        <select x-model="filters.book_id" @change="loadAssets(1)" class="rounded-lg border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        ...
        <select x-model="filters.status" @change="loadAssets(1)" class="rounded-lg border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        ...
        <select x-model="filters.category" @change="loadAssets(1)" class="rounded-lg border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
          ...
          <div class="bg-white rounded-lg border border-orange-200 p-4">
            <div class="flex items-start justify-between mb-2">
              <div>
                ...
                <div class="text-sm text-slate-500" x-text="formatCategory(asset.category)"></div>
              </div>
          ...
          <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-card hover:shadow-lg transition-shadow">
            ...
            <div class="flex items-start justify-between mb-4">
              <div>
                ...
                <div class="text-sm text-slate-600" x-text="formatCategory(asset.category)"></div>
              </div>
              <div>
            ...
            <div class="flex justify-between items-center mt-4">
              ...
              <div class="flex items-center gap-2">
                ...
                <button @click="approveAsset(asset.id)" 
                        x-show="asset.status === 'draft'" 
                        class="text-emerald-600 hover:text-emerald-800" 
                        title="Approve & Posting Jurnal">
                ...
                <button @click="editAsset(asset.id)" 
                        x-show="asset.status === 'draft'" 
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-slate-50 border-b border-slate-200">
            <tr>
              <th class="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Aset</th>
              <th class="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Kategori</th>
              <th class="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Tanggal Perolehan</th>
              <th class="px-4 py-3 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Nilai Perolehan</th>
              <th class="px-4 py-3 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Akumulasi Penyusutan</th>
              <th class="px-4 py-3 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Nilai Buku</th>
          ...
          <tbody class="bg-white divide-y divide-slate-200">
            <template x-for="asset in assetsData" :key="asset.id">
              <tr class="hover:bg-slate-50">
                <td class="px-4 py-4">
                  <div>
                    <div class="font-medium text-slate-900" x-text="asset.name"></div>
                    <div class="text-sm text-slate-500 font-mono" x-text="asset.code"></div>
                    <div class="text-xs text-slate-400" x-text="asset.location"></div>
                  </div>
                </td>
                <td class="px-4 py-4">
                  <span class="text-sm text-slate-600" x-text="formatCategory(asset.category)"></span>
                </td>
                <td class="px-4 py-4">
                  <span class="text-sm text-slate-600" x-text="asset.acquisition_date_formatted"></span>
                </td>
                <td class="px-4 py-4 text-right">
                  <span class="text-sm font-medium text-slate-900" x-text="formatCurrency(asset.acquisition_cost)"></span>
                </td>
                <td class="px-4 py-4 text-right">
                  <span class="text-sm font-medium text-red-600" x-text="formatCurrency(asset.accumulated_depreciation)"></span>
                </td>
                <td class="px-4 py-4 text-right">
                  <span class="text-sm font-bold text-blue-600" x-text="formatCurrency(asset.book_value)"></span>
                </td>
    ...
    <div class="flex items-center justify-between bg-white px-4 py-3 border border-slate-200 rounded-xl mt-6">
      <div class="flex items-center gap-4">
        <div class="text-sm text-slate-700">
          Menampilkan <span class="font-medium" x-text="((pagination.currentPage - 1) * pagination.perPage) + 1"></span> 
          sampai <span class="font-medium" x-text="Math.min(pagination.currentPage * pagination.perPage, pagination.total)"></span> 
          dari <span class="font-medium" x-text="pagination.total"></span> data
        </div>
        
        <select @change="changePerPage($event.target.value)" :value="pagination.perPage" 
      ...
      <div class="flex items-center gap-2">
        <button @click="prevPage()" 
                ...
                :class="pagination.currentPage <= 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-50'"
        ...
        <div class="flex items-center gap-1">
          <template x-for="page in (() => {
            ...
            <button x-show="page !== '...'" 
                    @click="goToPage(page)" 
                    :class="page === pagination.currentPage ? 'bg-blue-500 text-white' : 'hover:bg-slate-50'"
                    class="px-3 py-2 rounded-lg border border-slate-200 text-sm min-w-[40px]"
                    x-text="page">
            </button>
            <span x-show="page === '...'" class="px-2 py-2 text-slate-400">...</span>
          </template>
        </div>
        
        <button @click="nextPage()" 
                :disabled="pagination.currentPage >= pagination.lastPage"
                :class="pagination.currentPage >= pagination.lastPage ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-50'"
                class="px-3 py-2 rounded-lg border border-slate-200 text-sm">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-card overflow-hidden">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-6 border-b border-slate-200">
        ...
        <div class="flex flex-wrap gap-2">
          <select x-model="depreciationFilters.asset_id" @change="loadDepreciationHistory()" class="rounded-xl border border-slate-200 px-3 py-2 text-sm">
            ...
            <template x-for="asset in allAssetsData" :key="asset.id">
              <option :value="asset.id" x-text="asset.display_name"></option>
            </template>
          </select>
      ...
      <div x-show="selectedDepreciations.length > 0" class="bg-blue-50 border-b border-blue-200 px-6 py-3">
        <div class="flex items-center justify-between">
          <div class="text-sm text-blue-800">
            <span x-text="selectedDepreciations.length"></span> riwayat terpilih
          </div>
          <div class="flex gap-2">
            <button @click="bulkPostDepreciations()" 
                    x-show="getSelectedDraftCount() > 0"
                    class="inline-flex items-center gap-2 rounded-lg bg-green-600 text-white px-3 py-2 text-sm hover:bg-green-700">
              <i class='bx bx-check'></i> 
              Posting (<span x-text="getSelectedDraftCount()"></span>)
            </button>
            <button @click="bulkDeleteDepreciations()" 
                    class="inline-flex items-center gap-2 rounded-lg bg-red-600 text-white px-3 py-2 text-sm hover:bg-red-700">
              <i class='bx bx-trash'></i> 
              Hapus (<span x-text="selectedDepreciations.length"></span>)
            </button>
          </div>
        </div>
      </div>
          ...
          <thead class="bg-slate-50">
            <tr>
              <th class="px-4 py-3 text-left w-12">
                <input type="checkbox" 
                       @change="toggleAllDepreciations($event.target.checked)"
                       :checked="depreciationHistory.length > 0 && selectedDepreciations.length === depreciationHistory.length"
                       :indeterminate="selectedDepreciations.length > 0 && selectedDepreciations.length < depreciationHistory.length"
          ...
          <tbody>
            <template x-for="(depreciation, index) in depreciationHistory" :key="depreciation.id">
              <tr class="border-t border-slate-100 hover:bg-slate-50" 
                  :class="selectedDepreciations.includes(depreciation.id) ? 'bg-blue-50' : ''">
                <td class="px-4 py-3">
                  <input type="checkbox" 
                         :value="depreciation.id"
                         @change="toggleDepreciationSelection(depreciation.id, $event.target.checked)"
                         :checked="selectedDepreciations.includes(depreciation.id)"
                         class="rounded border-slate-300">
                </td>
    ...
    <div x-show="showDisposalModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center p-4 pt-20 z-50">
      <div class="bg-white rounded-2xl w-full max-w-2xl">
        ...
        <div class="p-6 space-y-4">
          ...
          <div>
            ...
            <input type="number" x-model="disposalForm.disposal_value" 
                   @input="calculateDisposalGainLoss()" 
                   class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" 
                   placeholder="0">
            <p class="text-xs text-blue-600 mt-1" x-show="disposalForm.disposal_value > 0" 
               x-text="'≈ ' + formatCurrency(disposalForm.disposal_value)"></p>
          </div>
    ...
    <div x-show="showBookSelectionModal" 
      ...
      <div class="bg-white rounded-2xl w-full max-w-md shadow-2xl">
        <div class="p-6 border-b border-slate-200">
          <div class="flex items-center justify-between">
            <div>
              <h3 class="text-lg font-semibold text-slate-800">Pilih Buku Akuntansi</h3>
              <p class="text-sm text-slate-600 mt-1">Pilih buku akuntansi untuk posting penyusutan</p>
            </div>
            <button @click="cancelBookSelection()" class="text-slate-400 hover:text-slate-600">
        ...
        <div class="p-6 space-y-4">
          ...
          <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="text-sm text-blue-800 font-medium mb-1">Informasi Posting:</p>
                <div class="text-sm text-blue-700 space-y-1">
                  <div x-show="bookSelectionType === 'single'">
                    <span>Posting 1 riwayat penyusutan</span>
                  </div>
                  <div x-show="bookSelectionType === 'bulk'">
                    <span>Posting <span x-text="selectedDepreciations.length"></span> riwayat penyusutan</span>
                  </div>
                  <div>Jurnal akan dibuat otomatis setelah posting</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-2">
              Pilih Buku Akuntansi <span class="text-red-500">*</span>
            </label>
            <select x-model="selectedBookForPosting" 
              ...
              <template x-for="book in books" :key="book.id">
                <option :value="book.id" x-text="book.name + ' (' + book.type + ')'"></option>
              </template>
            </select>
            <p class="text-xs text-slate-500 mt-1">Jurnal penyusutan akan dibuat di buku akuntansi yang dipilih</p>
          </div>

          
          <div x-show="selectedBookForPosting" class="bg-green-50 rounded-lg p-3 border border-green-200">
            <div class="flex items-center gap-2">
              <i class="bx bx-check-circle text-green-600"></i>
              <div class="text-sm">
                <span class="font-medium text-green-800">Buku Terpilih:</span>
                <span class="text-green-700" x-text="books.find(b => b.id == selectedBookForPosting)?.name"></span>
              </div>
            </div>
          </div>

          
          <div x-show="books.length === 0" class="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
            <div class="flex items-start gap-3">
              <i class="bx bx-exclamation-triangle text-yellow-600 text-xl"></i>
              <div class="flex-1">
                <p class="text-sm text-yellow-800 font-medium">Tidak Ada Buku Akuntansi</p>
                <p class="text-sm text-yellow-700 mt-1">Tidak ada buku akuntansi aktif untuk outlet ini. Hubungi administrator untuk membuat buku akuntansi.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="p-6 border-t border-slate-200 flex justify-end gap-3">
          <button @click="cancelBookSelection()" 
                  class="px-4 py-2 text-sm text-slate-600 hover:text-slate-800 border border-slate-200 rounded-lg hover:bg-slate-50">
            Batal
          </button>
          <button @click="confirmBookSelection()" 
                  :disabled="!selectedBookForPosting || books.length === 0"
                  :class="!selectedBookForPosting || books.length === 0 ? 'opacity-50 cursor-not-allowed bg-slate-400' : 'hover:bg-blue-700 bg-blue-600'"
                  class="px-4 py-2 text-white text-sm rounded-lg transition-colors">
            <span x-show="bookSelectionType === 'single'">
              <i class="bx bx-check mr-1"></i>Posting Penyusutan
            </span>
            <span x-show="bookSelectionType === 'bulk'">
              <i class="bx bx-check-double mr-1"></i>Posting Semua
            </span>
          </button>
        </div>
      </div>
    ...
    <div x-show="showViewModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center p-4 pt-20 z-50 overflow-y-auto" @click.self="showViewModal = false">
    ...
    <div x-show="showDepreciationModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center p-4 pt-20 z-50">
      <div class="bg-white rounded-2xl w-full max-w-2xl">
        <div class="p-6 border-b border-slate-200">
          <h3 class="text-lg font-semibold text-slate-800">Hitung Penyusutan Batch</h3>
          <p class="text-sm text-slate-600 mt-1">Sistem akan menghitung semua penyusutan yang belum dihitung mulai dari tanggal perolehan hingga periode yang dipilih</p>
        </div>
        ...
        <div class="p-6 space-y-4">
          <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
            <div class="flex items-start gap-3">
              <i class='bx bx-info-circle text-blue-600 text-xl'></i>
              <div class="flex-1">
                <p class="text-sm text-blue-800 font-medium mb-1">Cara Kerja Sistem:</p>
                <ul class="text-sm text-blue-700 space-y-1">
                  <li>• Aset dengan tanggal perolehan 1 Nov 2025</li>
                  <li>• Hitung penyusutan Jan 2026</li>
                  <li>• Sistem akan buat penyusutan Des 2025 & Jan 2026</li>
                  <li>• Penyusutan dimulai bulan setelah perolehan</li>
                </ul>
              </div>
            </div>
          </div>
    ...
    <div x-show="showAssetModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center p-4 pt-10 z-50 overflow-y-auto">
          ...
          <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select x-model="assetForm.book_id" class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                  <option value="">Pilih Buku Akuntansi</option>
                  <template x-for="book in books" :key="book.id">
                    <option :value="book.id" x-text="book.name"></option>
                  </template>
                </select>
                <p class="text-xs text-slate-500 mt-1">Pilih buku akuntansi untuk asset ini</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" x-model="assetForm.acquisition_cost" 
                       @input="updateAcquisitionCostDisplay()"
                       class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" 
                       placeholder="0">
                <p class="text-xs text-blue-600 mt-1" x-show="assetForm.acquisition_cost > 0" 
                   x-text="'≈ ' + formatCurrency(assetForm.acquisition_cost)"></p>
              </div>
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Nilai Residu</label>
                <input type="number" x-model="assetForm.salvage_value" 
                       @input="updateSalvageValueDisplay()"
                       class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" 
                       placeholder="0">
                <p class="text-xs text-blue-600 mt-1" x-show="assetForm.salvage_value > 0" 
                   x-text="'≈ ' + formatCurrency(assetForm.salvage_value)"></p>
              </div>
              <div>
    ...
    <div x-show="showImportModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center p-4 pt-20 z-50" @click.self="showImportModal = false">
      <div class="bg-white rounded-2xl w-full max-w-3xl">
        ...
        <div class="p-6 space-y-4">
          ...
          <div>
            ...
            <select x-model="importForm.book_id" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
              <option value="">Pilih Buku Akuntansi</option>
              <template x-for="book in books" :key="book.id">
                <option :value="book.id" x-text="book.name + ' (' + book.type + ')'"></option>
              </template>
            </select>
            <p class="text-xs text-slate-500 mt-1">Semua aktiva tetap yang diimport akan menggunakan buku akuntansi ini</p>
          </div>
  ...
  <script>
    function fixedAssetsManagement() {
      return {
        ...
        depreciationForm: {
          outlet_id: null, // Will be set in init()
        ...
        filters: {
          ...
          book_id: '', // Empty = all books (Semua Buku)
          status: 'all',
          category: 'all'
        },
        outlets: [],
        ...
        assetsData: [],
        allAssetsData: [], // All assets for dropdown filter (not paginated)
        ...
        bookSelectionType: 'single', // 'single' or 'bulk'
        pendingDepreciationId: null,
        ...
        async init() {
          ...
          const savedViewMode = localStorage.getItem('fixed_assets_view_mode');
          ...
          try {
            ...
            // Set outlet_id to depreciationForm after outlets loaded
            this.depreciationForm.outlet_id = this.filters.outlet_id;
            console.log('Set depreciationForm.outlet_id to:', this.depreciationForm.outlet_id);
            ...
            await Promise.all([
              this.loadAssets(),
              this.loadAllAssets(), // Load all assets for dropdown filter
        ...
        async loadBooks() {
          ...
          try {
            const response = await fetch(`<?php echo e(route('finance.active-books.data')); ?>?outlet_id=${this.filters.outlet_id}`);
            const result = await response.json();
            if (result.success) {
              this.books = result.data;
              // Tidak set default book_id lagi, biarkan kosong untuk "Semua Buku"
        ...
        async onOutletChange() {
          ...
          // Update depreciationForm outlet_id
          this.depreciationForm.outlet_id = this.filters.outlet_id;
          
          console.log('Outlet changed to:', this.filters.outlet_id);
          console.log('Updated depreciationForm.outlet_id to:', this.depreciationForm.outlet_id);
          ...
          // Reload all data for new outlet (reset to page 1)
          await Promise.all([
            this.loadAssets(1),
            this.loadAllAssets(), // Reload all assets for dropdown filter
            this.loadAccounts(), // Reload accounts for new outlet
            this.loadDepreciationHistory()
          ]);
        ...
        resetFilters() {
          ...
          <?php if(auth()->user() && auth()->user()->id_outlet): ?>
            const defaultOutlet = <?php echo e(auth()->user()->id_outlet); ?>;
          <?php else: ?>
        ...
        async loadOutlets() {
          try {
            ...
            if (result.success) {
              ...
              <?php if(auth()->user() && auth()->user()->id_outlet): ?>
                this.filters.outlet_id = <?php echo e(auth()->user()->id_outlet); ?>;
              <?php else: ?>
        ...
        async loadAssets(page = 1) {
          try {
            ...
            // Hanya tambahkan book_id jika tidak kosong (kosong = Semua Buku)
            ...
            const response = await fetch(`<?php echo e(route("finance.fixed-assets.data")); ?>?${params.toString()}`, {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
            ...
            if (result.success) {
              ...
              console.log('Assets loaded successfully:', this.assetsData.length, 'items');
        ...
        async loadAllAssets() {
          try {
            ...
            const response = await fetch(`<?php echo e(route("finance.fixed-assets.assets.all")); ?>?${params.toString()}`, {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
            ...
            if (result.success) {
              // Force reactive update by creating new array
              this.allAssetsData = [...result.data];
              console.log('All assets loaded for dropdown:', this.allAssetsData.length, 'items');
            } else {
              console.error('Failed to load all assets:', result.message);
              this.allAssetsData = [];
            }
          } catch (error) {
        ...
        async changePerPage(perPage) {
          ...
          await this.loadAssets(1); // Reset to first page
        },

        // View mode methods
        switchViewMode(mode) {
          this.viewMode = mode;
          localStorage.setItem('fixed_assets_view_mode', mode);
        },

        async forceRefreshAssets() {
          console.log('Force refreshing assets data...');
          ...
          params.append('_cache_bust', Math.random().toString(36));
          ...
          try {
            ...
            if (result.success) {
              // Force complete replacement of data
              this.assetsData = result.data.map(asset => ({...asset}));
              this.assetStats = {...result.stats};
              ...
              console.log('Force refresh successful:', this.assetsData.length, 'items');
              console.log('Sample refreshed asset:', this.assetsData[0]);
              
              // Force Alpine.js to re-render
              this.$nextTick(() => {
                console.log('Alpine.js re-render triggered');
              });
        ...
        async loadDepreciationHistory() {
          try {
            ...
            const url = `<?php echo e(route("finance.fixed-assets.depreciation.history")); ?>?${params.toString()}`;
        ...
        async loadAccounts() {
          try {
            ...
            // Get outlet_id from current filter
            const outletId = this.filters.outlet_id || (this.outlets[0]?.id_outlet || null);
            console.log('Loading accounts for outlet:', outletId);
            ...
            const [assetResult, expenseResult, contraResult, paymentResult] = await Promise.all([
              fetch(`<?php echo e(route("finance.accounts.active")); ?>?outlet_id=${outletId}&type=asset&exclude_contra=true`, { method: 'GET', headers }).then(r => r.json()),
              fetch(`<?php echo e(route("finance.accounts.active")); ?>?outlet_id=${outletId}&type=expense`, { method: 'GET', headers }).then(r => r.json()),
              fetch(`<?php echo e(route("finance.accounts.active")); ?>?outlet_id=${outletId}&type=asset&contra=true`, { method: 'GET', headers }).then(r => r.json()),
              fetch(`<?php echo e(route("finance.accounts.active")); ?>?outlet_id=${outletId}&type=asset&exclude_contra=true`, { method: 'GET', headers }).then(r => r.json())
            ]);

            console.log('Account loading results:', {
              asset: assetResult.success ? assetResult.data.length : 'failed',
              expense: expenseResult.success ? expenseResult.data.length : 'failed',
              contra: contraResult.success ? contraResult.data.length : 'failed',
              payment: paymentResult.success ? paymentResult.data.length : 'failed'
            });

            // Process results
            if (assetResult.success) {
              this.assetAccounts = assetResult.data;
              console.log('Asset accounts loaded:', this.assetAccounts.length);
            } else {
              console.error('Failed to load asset accounts:', assetResult.message);
              this.assetAccounts = [];
            }
            
            if (expenseResult.success) {
              this.depreciationAccounts = expenseResult.data;
              console.log('Expense accounts loaded:', this.depreciationAccounts.length);
            } else {
              console.error('Failed to load expense accounts:', expenseResult.message);
              this.depreciationAccounts = [];
            }
            
            if (contraResult.success) {
              this.accumulatedDepreciationAccounts = contraResult.data;
              console.log('Contra accounts loaded:', this.accumulatedDepreciationAccounts.length);
            } else {
              console.error('Failed to load contra accounts:', contraResult.message);
            ...
            if (paymentResult.success) {
              ...
              console.log('Payment accounts loaded:', this.paymentAccounts.length);
            } else {
              console.error('Failed to load payment accounts:', paymentResult.message);
        ...
        async loadChartData() {
          try {
            ...
            const valueResponse = await fetch(`<?php echo e(route("finance.fixed-assets.chart.value")); ?>?period=${this.chartPeriod}${outletParam}`, {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            const distributionResponse = await fetch(`<?php echo e(route("finance.fixed-assets.chart.distribution")); ?>?${outletParam.substring(1)}`, {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        async openCreateAsset() {
          ...
          const outletId = this.filters.outlet_id === 'all' ? (this.outlets[0]?.id_outlet || null) : this.filters.outlet_id;
          
          console.log('Opening create asset modal with outlet_id:', outletId);
          
          // Generate asset code from API
          try {
            const response = await fetch(`<?php echo e(route("finance.fixed-assets.generate-code")); ?>?outlet_id=${outletId}`, {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            this.assetForm = {
              code: generatedCode,
              outlet_id: outletId,
              book_id: '', // Kosong, user harus pilih manual
              name: '',
              category: 'equipment',
              location: '',
              acquisition_date: new Date().toISOString().split('T')[0],
          ...
          console.log('Asset form initialized with empty book_id for manual selection');
        ...
        async editAsset(id) {
          try {
            ...
            const response = await fetch(`<?php echo e(route("finance.fixed-assets.show", ":id")); ?>`.replace(':id', id), {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            if (result.success) {
              ...
              this.$nextTick(() => {
                this.assetForm = {
                  ...
                  book_id: asset.book_id || '', // Set book_id dari data asset
                  code: asset.code || '',
                  name: asset.name || '',
                ...
                // Add form change listeners for debugging
                this.$watch('assetForm.name', (newVal, oldVal) => {
                  console.log('Name changed from:', oldVal, 'to:', newVal);
                });
                this.$watch('assetForm.location', (newVal, oldVal) => {
                  console.log('Location changed from:', oldVal, 'to:', newVal);
                });
                this.$watch('assetForm.salvage_value', (newVal, oldVal) => {
                  console.log('Salvage value changed from:', oldVal, 'to:', newVal);
                });
                this.$watch('assetForm.acquisition_cost', (newVal, oldVal) => {
                  console.log('Acquisition cost changed from:', oldVal, 'to:', newVal);
                });
                this.$watch('assetForm.asset_account_id', (newVal, oldVal) => {
                  console.log('Asset account changed from:', oldVal, 'to:', newVal);
                });
                this.$watch('assetForm.payment_account_id', (newVal, oldVal) => {
                  console.log('Payment account changed from:', oldVal, 'to:', newVal);
                });
        ...
        updateAcquisitionCostDisplay() {
          // Force Alpine.js to update the display
          this.$nextTick(() => {
            // This will trigger the x-text update for the currency display
          });
        },

        updateSalvageValueDisplay() {
          // Force Alpine.js to update the display
          this.$nextTick(() => {
            // This will trigger the x-text update for the currency display
          });
        ...
        async saveAsset() {
          ...
          // Validate book_id - sekarang wajib dipilih
          if (!this.assetForm.book_id) {
            alert('Buku akuntansi harus dipilih');
            return;
          }
          ...
          // Only validate accounts for new assets, not for editing
          ...
          try {
            const url = this.editingAsset 
              ? `<?php echo e(route('finance.fixed-assets.update', ':id')); ?>`.replace(':id', this.editingAsset)
              : '<?php echo e(route('finance.fixed-assets.store')); ?>';
            
            // Prepare data - different fields based on asset status
            let data;
            let currentAsset = null;
            let isDraft = false;
            
            if (this.editingAsset) {
              // Get current asset status to determine which fields can be updated
              currentAsset = this.assetsData.find(asset => asset.id === this.editingAsset);
              isDraft = currentAsset && currentAsset.status === 'draft';
              
              // Base fields that can always be updated
              data = {
                book_id: this.assetForm.book_id,
                name: this.assetForm.name,
                category: this.assetForm.category,
                location: this.assetForm.location,
                salvage_value: this.assetForm.salvage_value,
                useful_life: this.assetForm.useful_life,
                depreciation_method: this.assetForm.depreciation_method,
                description: this.assetForm.description,
                _method: 'PUT'
              };
              
              // For draft assets, also send acquisition cost and accounts
              if (isDraft) {
                data.acquisition_cost = this.assetForm.acquisition_cost;
                data.asset_account_id = this.assetForm.asset_account_id;
                data.depreciation_expense_account_id = this.assetForm.depreciation_expense_account_id;
                data.accumulated_depreciation_account_id = this.assetForm.accumulated_depreciation_account_id;
                data.payment_account_id = this.assetForm.payment_account_id;
              }
            } else {
            ...
            console.log('Edit mode:', !!this.editingAsset);
            console.log('Current asset status:', currentAsset ? currentAsset.status : 'N/A');
            console.log('Is draft:', isDraft);
            console.log('Asset form data:', JSON.stringify(this.assetForm, null, 2));
            console.log('Prepared data for API:', JSON.stringify(data, null, 2));
            console.log('========================');
            ...
            if (result.success) {
              console.log('Save successful, response data:', result.data);
              alert(this.editingAsset ? 'Aset berhasil diperbarui' : 'Aset berhasil ditambahkan');
              this.closeAssetModal();
              
              // Use force refresh to ensure data is updated
              await this.forceRefreshAssets();
            } else {
        ...
        async processBatchDepreciation() {
          if (!confirm('Apakah Anda yakin ingin menghitung penyusutan untuk periode ini? Sistem akan menghitung semua penyusutan yang belum dihitung mulai dari tanggal perolehan hingga periode yang dipilih.')) {
            return;
          }

          try {
            this.depreciationProgress.show = true;
            this.depreciationProgress.message = 'Menghitung penyusutan dari tanggal perolehan...';
            this.depreciationProgress.processed = 0;
            this.depreciationProgress.total = 0;

            const response = await fetch('<?php echo e(route("finance.fixed-assets.depreciation.batch")); ?>', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            if (result.success) {
              ...
              this.depreciationProgress.processed = result.data.total_depreciation_entries || result.data.total_assets_processed;
              this.depreciationProgress.total = result.data.total_depreciation_entries || result.data.total_assets_processed;
              
              // Show detailed summary
              let summary = `Penyusutan berhasil dihitung:\n`;
              summary += `- Total aset diproses: ${result.data.total_assets_processed}\n`;
              summary += `- Total entri penyusutan dibuat: ${result.data.total_depreciation_entries || 0}\n`;
              
              if (result.data.total_journals_created > 0) {
                summary += `- Total jurnal diposting: ${result.data.total_journals_created}\n`;
              }
              
              summary += `- Total nilai penyusutan: ${this.formatCurrency(result.data.total_depreciation_amount)}\n`;
              
              if (result.data.errors && result.data.errors.length > 0) {
                summary += `\nError:\n`;
                result.data.errors.forEach(error => {
                  summary += `- ${error.asset_code}${error.period ? ' (' + error.period + ')' : ''}: ${error.error}\n`;
                });
              }
              
              if (result.data.total_depreciation_entries > result.data.total_assets_processed) {
                summary += `\nCatatan: Sistem telah menghitung penyusutan untuk beberapa periode sekaligus berdasarkan tanggal perolehan masing-masing aset.`;
              }
        ...
        async toggleAsset(id, status) {
          ...
          try {
            const response = await fetch(`<?php echo e(route("finance.fixed-assets.toggle", ":id")); ?>`.replace(':id', id), {
              method: 'PATCH',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        async approveAsset(id) {
          if (!confirm('Apakah Anda yakin ingin approve aktiva tetap ini? Jurnal perolehan akan diposting secara otomatis.')) {
            return;
          }

          try {
            const response = await fetch(`<?php echo e(route("finance.fixed-assets.approve", ":id")); ?>`.replace(':id', id), {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            if (result.success) {
              alert('Aktiva tetap berhasil diapprove dan jurnal telah diposting');
              await this.loadAssets();
            } else {
              alert('Gagal approve aktiva tetap: ' + result.message);
            }
          } catch (error) {
            console.error('Error approving asset:', error);
            alert('Terjadi kesalahan saat approve aktiva tetap');
          }
        },
        ...
        async disposeAsset() {
          ...
          try {
            const response = await fetch(`<?php echo e(route("finance.fixed-assets.dispose", ":id")); ?>`.replace(':id', this.disposalAsset.id), {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        async postDepreciation(id) {
          console.log('🔄 Posting depreciation with ID:', id);
          ...
          if (!this.books || this.books.length === 0) {
            alert('Tidak ada buku akuntansi aktif untuk outlet ini. Hubungi administrator untuk membuat buku akuntansi.');
            return;
          }

          // Show book selection modal
          this.openBookSelectionModal('single', id);
        },

        async executePostDepreciation(id, bookId) {
          console.log('🔄 Executing post depreciation:', { id, bookId });
          
          try {
            const response = await fetch(`<?php echo e(route("finance.fixed-assets.depreciation.post", ":id")); ?>`.replace(':id', id), {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            const result = await response.json();
            console.log('📊 Post depreciation result:', result);
            
            if (result.success) {
              const selectedBook = this.books.find(book => book.id == bookId);
              const bookName = selectedBook ? selectedBook.name : 'Buku Akuntansi';
              
              alert(`Penyusutan berhasil diposting ke ${bookName} dan jurnal telah dibuat`);
              await this.loadDepreciationHistory();
              await this.loadAssets();
            } else {
            ...
            alert('Terjadi kesalahan saat memposting penyusutan: ' + error.message);
          }
        },
        ...
        async reverseDepreciation(id) {
          ...
          try {
            const response = await fetch(`<?php echo e(route("finance.fixed-assets.depreciation.reverse", ":id")); ?>`.replace(':id', id), {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        exportToXLSX() {
          ...
          window.location.href = `<?php echo e(route("finance.fixed-assets.export.xlsx")); ?>?${params.toString()}`;
        },
        ...
        exportToPDF() {
          ...
          window.open(`<?php echo e(route("finance.fixed-assets.export.pdf")); ?>?${params.toString()}`, '_blank');
        },
        ...
        async downloadTemplate() {
          try {
            window.location.href = '<?php echo e(route("finance.fixed-assets.template")); ?>';
          } catch (error) {
        ...
        async processImport() {
          ...
          if (!this.importForm.book_id) {
            alert('Pilih buku akuntansi terlebih dahulu');
            return;
          }
          ...
          formData.append('book_id', this.importForm.book_id);
          ...
          try {
            ...
            const response = await fetch('<?php echo e(route("finance.fixed-assets.import")); ?>', {
              method: 'POST',
              body: formData,
              headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        toggleDepreciationSelection(id, checked) {
          if (checked) {
            if (!this.selectedDepreciations.includes(id)) {
              this.selectedDepreciations.push(id);
            }
          } else {
            this.selectedDepreciations = this.selectedDepreciations.filter(selectedId => selectedId !== id);
          }
          console.log('Selected depreciations:', this.selectedDepreciations);
        },

        toggleAllDepreciations(checked) {
          if (checked) {
            this.selectedDepreciations = [...this.depreciationHistory.map(d => d.id)];
          } else {
            this.selectedDepreciations = [];
          }
          console.log('Toggle all - Selected depreciations:', this.selectedDepreciations);
        },

        getSelectedDraftCount() {
          return this.selectedDepreciations.filter(id => {
            const depreciation = this.depreciationHistory.find(d => d.id === id);
            return depreciation && depreciation.status === 'draft';
          }).length;
        },

        async bulkPostDepreciations() {
          console.log('🔄 Bulk posting depreciations');
          
          const draftIds = this.selectedDepreciations.filter(id => {
            const depreciation = this.depreciationHistory.find(d => d.id === id);
            return depreciation && depreciation.status === 'draft';
          });

          console.log('📊 Draft IDs to post:', draftIds);

          if (draftIds.length === 0) {
            alert('Tidak ada riwayat penyusutan draft yang dipilih');
            return;
          }
          ...
          if (!confirm(`Apakah Anda yakin ingin memposting ${draftIds.length} riwayat penyusutan? Jurnal akan dibuat secara otomatis.`)) {
            return;
          }

          // Show book selection modal
          this.openBookSelectionModal('bulk');
        },

        async executeBulkPostDepreciations(bookId) {
          console.log('🔄 Executing bulk post depreciations:', { bookId });
          ...
          console.log('📊 Draft IDs for bulk posting:', draftIds);

          try {
            const response = await fetch('<?php echo e(route("finance.fixed-assets.depreciation.bulk-post")); ?>', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
              ...
              body: JSON.stringify({
                depreciation_ids: draftIds,
                book_id: bookId
              })
            });

            const result = await response.json();
            console.log('📊 Bulk post result:', result);
            
            if (result.success) {
              const successCount = result.data.success_count || draftIds.length;
              const failedCount = result.data.failed_count || 0;
              
              const selectedBook = this.books.find(book => book.id == bookId);
              const bookName = selectedBook ? selectedBook.name : 'Buku Akuntansi';
              
              let message = `${successCount} riwayat penyusutan berhasil diposting ke ${bookName}`;
              if (failedCount > 0) {
                message += `, ${failedCount} gagal diposting`;
              }
              ...
              alert('Gagal memposting riwayat penyusutan: ' + result.message);
            }
          } catch (error) {
            console.error('❌ Error bulk posting depreciations:', error);
            alert('Terjadi kesalahan saat memposting riwayat penyusutan: ' + error.message);
          }
        },

        async bulkDeleteDepreciations() {
          if (this.selectedDepreciations.length === 0) {
            alert('Tidak ada riwayat penyusutan yang dipilih');
            return;
          }

          if (!confirm(`PERINGATAN: Apakah Anda yakin ingin menghapus ${this.selectedDepreciations.length} riwayat penyusutan? Tindakan ini tidak dapat dibatalkan.`)) {
            return;
          }

          try {
            const response = await fetch('<?php echo e(route("finance.fixed-assets.depreciation.bulk-delete")); ?>', {
              method: 'DELETE',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
              ...
              body: JSON.stringify({
                depreciation_ids: this.selectedDepreciations
              })
            });
            ...
            if (result.success) {
              const deletedCount = result.data.deleted_count || this.selectedDepreciations.length;
              alert(`${deletedCount} riwayat penyusutan berhasil dihapus`);
              ...
              alert('Gagal menghapus riwayat penyusutan: ' + result.message);
            }
          } catch (error) {
            console.error('Error bulk deleting depreciations:', error);
            alert('Terjadi kesalahan saat menghapus riwayat penyusutan');
          }
        },

        // Book selection modal functions
        openBookSelectionModal(type, depreciationId = null) {
          console.log('📖 Opening book selection modal:', { type, depreciationId });
          console.log('📚 Available books:', this.books);
          
          this.bookSelectionType = type;
          this.pendingDepreciationId = depreciationId;
          this.selectedBookForPosting = '';
          
          // Auto-select first book if only one available
          if (this.books && this.books.length === 1) {
            this.selectedBookForPosting = this.books[0].id;
            console.log('🎯 Auto-selected single book:', this.books[0].name);
          }
          
          this.showBookSelectionModal = true;
          console.log('✅ Book selection modal opened');
        },

        cancelBookSelection() {
          console.log('❌ Cancelling book selection');
          this.showBookSelectionModal = false;
          this.selectedBookForPosting = '';
          this.bookSelectionType = 'single';
          this.pendingDepreciationId = null;
        },

        async confirmBookSelection() {
          console.log('✅ Confirming book selection:', this.selectedBookForPosting);
          
          if (!this.selectedBookForPosting) {
            alert('Pilih buku akuntansi terlebih dahulu');
            return;
          }

          const selectedBook = this.books.find(book => book.id == this.selectedBookForPosting);
          console.log('📖 Selected book details:', selectedBook);

          this.showBookSelectionModal = false;

          try {
            if (this.bookSelectionType === 'single') {
              console.log('🔄 Executing single depreciation posting...');
              await this.executePostDepreciation(this.pendingDepreciationId, this.selectedBookForPosting);
            } else if (this.bookSelectionType === 'bulk') {
              console.log('🔄 Executing bulk depreciation posting...');
              await this.executeBulkPostDepreciations(this.selectedBookForPosting);
            }
          } catch (error) {
            console.error('❌ Error in confirmBookSelection:', error);
            alert('Terjadi kesalahan saat memposting penyusutan: ' + error.message);
          } finally {
            this.cancelBookSelection();
          }
        }
      };
    }
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\hm\resources\views\admin\finance\aktiva-tetap\index.blade.php ENDPATH**/ ?>