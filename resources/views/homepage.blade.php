                ...
                <div class="flex gap-3 pt-2 px-2">
                    ...
                    <div class="flex-1 relative" id="mobile-login-wrapper">
                        ...
                        <div id="mobile-login-dropdown"
                            ...
                            <a href="{{ route('affiliate.login') }}" class="flex items-center gap-3 px-4 py-3 hover:bg-green-50 border-t border-gray-50">
...
<div id="hero-section">
    ...
    <img id="hero-illustration"
         src="{{ url('WEB_HMTour/wp-content/uploads/2025/07/Umroh-sesuai-sunnah.png') }}"
    ...
    <img id="hero-right-5pasti"
         ...
         src="{{ url('WEB_HMTour/wp-content/uploads/2025/08/5_pasti-removebg-preview.png') }}"
         alt="5 Pasti HM Tour"
         onerror="this.style.display='none'">
    ...
    <div class="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-8" style="margin-top:-20px;">
        <form action="{{ url('/') }}" method="GET" class="hero-search p-4 sm:p-5">
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-0">
                ...
                <div class="hero-search-item flex-1 px-4 py-2">
                    ...
                    <select id="s_outlet" name="outlet_id"
                        ...
                        <option value="{{ $outlet->id_outlet }}"
                            {{ request('outlet_id') == $outlet->id_outlet ? 'selected' : '' }}>
                ...
                <div class="flex-1 px-4 py-2">
                    ...
                    <select id="s_jemaah" name="jemaah"
                            class="w-full text-xs text-gray-500 bg-transparent border-none outline-none cursor-pointer appearance-none">
                        @for($j = 1; $j <= 10; $j++)
                        <option value="{{ $j }}" {{ request('jemaah', 1) == $j ? 'selected' : '' }}>{{ $j }} Orang</option>
                        @endfor
                    </select>
                </div>
                <!-- Search button -->
...
<div class="section-white py-6 overflow-hidden border-y border-green-100">
    ...
    <div class="overflow-hidden">
        <div class="marquee-track flex gap-14 items-center" style="width: max-content;">
            ...
            @endphp
            @foreach(array_merge($airlines, $airlines) as $airline)
            <div class="flex-shrink-0 opacity-50 hover:opacity-100 transition-opacity">
                <img src="{{ $airline['src'] }}" alt="{{ $airline['alt'] }}"
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h2 class="font-playfair text-2xl font-bold text-gray-900">
                    Hasil Pencarian
                    <span class="text-green-brand">({{ $searchResults->count() }} paket)</span>
                </h2>
                <p class="text-gray-500 text-sm mt-1">
                    @if(request('outlet_id') && ($selectedOutlet = $outlets->firstWhere('id_outlet', request('outlet_id'))))
                        Perusahaan: <strong>{{ $selectedOutlet->nama_outlet }}</strong>
                    @endif
                    @if(request('bulan'))
                        · Bulan: <strong>{{ ['','Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'][request('bulan')] }}</strong>
                    @endif
                    @if(request('package_type'))
                        · Program: <strong>{{ ucwords(str_replace('_', ' ', request('package_type'))) }}</strong>
                    @endif
                </p>
            </div>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="card-hover bg-white rounded-2xl overflow-hidden border border-green-100 shadow-sm group">
                <div class="bg-gradient-to-br from-green-50 to-white p-5 border-b border-green-100">
                    ...
                    <h3 class="font-bold text-gray-900 text-center text-base mb-2">{{ $program->name }}</h3>
                    <div class="text-center mb-3">
                        ...
                        @else
                            <span class="inline-block bg-amber-100 text-amber-700 text-xs font-bold px-3 py-1 rounded-full">{{ $program->formatted_fee }}</span>
                        @endif
                    ...
                    <p class="text-xs text-gray-600 text-center line-clamp-2 min-h-[32px]">{{ $program->description }}</p>
                </div>
                <div class="p-4">
                    <!-- Target Audience -->
                    <div class="mb-3 pb-3 border-b border-gray-100">
                        <div class="text-xs text-gray-500 mb-1">Target:</div>
                        <div class="text-xs font-semibold text-gray-700">{{ $program->target_audience }}</div>
                    </div>
                    ...
                    <div class="space-y-2 mb-4">
                        <div class="flex items-center justify-between text-xs">
                            ...
                            <span class="font-semibold text-green-600">{{ $program->formatted_commission }}</span>
                        </div>
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-gray-500">Komisi Per Click:</span>
                            <span class="font-semibold text-blue-600">Rp {{ number_format($program->default_ppc_commission, 0, ',', '.') }}</span>
                        </div>
                        @if($program->max_commission_per_sale > 0)
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-gray-500">Max Per Sale:</span>
                            <span class="font-semibold text-amber-600">Rp {{ number_format($program->max_commission_per_sale, 0, ',', '.') }}</span>
                        </div>
                        @endif
                    </div>
...
<section id="keunggulan" class="section-white py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-content relative">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="card-hover bg-white rounded-2xl p-6 text-center border border-green-100 shadow-sm hover:border-green-300">
                <div class="w-16 h-16 mx-auto mb-4 rounded-2xl bg-green-pale flex items-center justify-center overflow-hidden">
                    ...
                    <i class="{{ $item['icon'] }} text-green-brand text-2xl" style="display:none"></i>
                </div>
        ...
        <div class="mt-12 rounded-3xl overflow-hidden border border-green-200 shadow-sm">
            <div class="relative">
                <img src="{{ url('WEB_HMTour/wp-content/uploads/2025/08/Akreditasi-A-HM-Tour-Oleh-Kemenag.jpeg') }}"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-4">
                    <div class="rounded-2xl overflow-hidden border-2 border-white shadow-md h-48">
                        <img src="{{ url('WEB_HMTour/wp-content/uploads/2025/08/Jemaah-HM-Tour-plus-turki.jpeg') }}"
                    ...
                    <div class="rounded-2xl overflow-hidden border-2 border-white shadow-md h-32">
                        <img src="{{ url('WEB_HMTour/wp-content/uploads/2025/08/Mitra-Stakeholder-HM-Tour-Travel-umroh-sunnah.jpeg') }}"
                ...
                <div class="space-y-4 mt-8">
                    <div class="rounded-2xl overflow-hidden border-2 border-white shadow-md h-32">
                        <img src="{{ url('WEB_HMTour/wp-content/uploads/2025/08/Masjidil-Haram-Makkah-Umroh-Sesuai-Sunnah.jpeg') }}"
                             alt="Masjidil Haram" class="w-full h-full object-cover"
                             onerror="this.parentElement.style.background='#e8f5e9';this.remove()">
                    </div>
                    <div class="rounded-2xl overflow-hidden border-2 border-white shadow-md h-48">
                        <img src="{{ url('WEB_HMTour/wp-content/uploads/2025/08/Masjid-Nabawi-Madinah-Umroh-Sesuai-Sunnah.jpeg') }}"
            ...
            <div>
                ...
                <div class="flex items-center gap-4 p-4 bg-white rounded-2xl border border-green-100 shadow-sm">
                    <div class="w-14 h-14 rounded-full overflow-hidden border-2 border-green-200 flex-shrink-0">
                        <img src="{{ url('WEB_HMTour/wp-content/uploads/2025/08/Husni-Mubarok-General-Manager-HM-Tour.jpeg') }}"
                             alt="Husni Mubarok"
...
<section class="section-white py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-content relative">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <a href="https://www.google.com/search?q={{ urlencode($hotel['name'] . ' ' . $hotel['city'] . ' Saudi Arabia') }}" 
               target="_blank" 
            ...
            <div class="testimonial-card rounded-2xl p-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-green-brand text-xs font-medium">{{ $t['package'] }}</div>
                    </div>
                </div>
            </div>
            @endforeach
        ...
        <div class="bg-gradient-to-br from-green-50 to-white rounded-3xl border-2 border-green-200 shadow-2xl overflow-hidden">
            <div class="p-8 lg:p-12">
                ...
                <div class="grid sm:grid-cols-2 gap-4 mb-8">
                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $affiliator->phone_number) }}" target="_blank"
                       class="bg-white border-2 border-green-200 hover:border-green-500 rounded-2xl p-6 transition-all hover:shadow-lg group">
                        ...
                        <div class="text-gray-600 text-sm">{{ $affiliator->phone_number }}</div>
                    </a>
                    
                    <a href="mailto:{{ $affiliator->email }}"
                       class="bg-white border-2 border-green-200 hover:border-green-500 rounded-2xl p-6 transition-all hover:shadow-lg group">
                        <div class="w-14 h-14 bg-green-100 rounded-xl flex items-center justify-center mx-auto mb-3 group-hover:bg-green-500 transition-colors">
                        ...
                        <div class="text-gray-600 text-sm break-all">{{ $affiliator->email }}</div>
                    </a>
                </div>

                <!-- CTA Button -->
                <div class="text-center">
                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $affiliator->phone_number) }}?text=Assalamu'alaikum {{ urlencode($affiliator->full_name) }}, saya ingin konsultasi paket umroh HM Tour"
                       target="_blank"
                       class="inline-flex items-center gap-3 bg-green-gradient text-white font-bold px-10 py-4 rounded-full text-base transition-all shadow-xl shadow-green-300 hover:opacity-90">
                        ...
                        Konsultasi dengan {{ explode(' ', $affiliator->full_name)[0] }}
                    </a>
...
<section id="kontak" class="section-pale py-20 relative overflow-hidden">
    ...
    <div class="relative z-content max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div class="grid sm:grid-cols-2 gap-4 mb-10 max-w-2xl mx-auto">
            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $affiliator->phone_number) }}" target="_blank"
               class="bg-white border-2 border-green-200 hover:border-green-brand rounded-2xl p-6 transition-all hover:shadow-md group">
                ...
                <div class="font-semibold text-gray-900 text-sm mb-1">WhatsApp {{ explode(' ', $affiliator->full_name)[0] }}</div>
                <div class="text-gray-600 text-xs">{{ $affiliator->phone_number }}</div>
            </a>
            <a href="mailto:{{ $affiliator->email }}"
               class="bg-white border-2 border-green-200 hover:border-green-brand rounded-2xl p-6 transition-all hover:shadow-md group">
                <div class="w-12 h-12 bg-green-100 rounded-xl flex items-center justify-center mx-auto mb-3 group-hover:bg-green-500 transition-colors">
                ...
                <div class="font-semibold text-gray-900 text-sm mb-1">Email {{ explode(' ', $affiliator->full_name)[0] }}</div>
                <div class="text-gray-600 text-xs break-all">{{ $affiliator->email }}</div>
            </a>
        </div>
        ...
        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $affiliator->phone_number) }}?text=Assalamu'alaikum {{ urlencode($affiliator->full_name) }}, saya ingin konsultasi paket umroh HM Tour"
           ...
           class="inline-flex items-center gap-3 bg-green-gradient text-white font-bold px-10 py-4 rounded-full text-base transition-all shadow-lg shadow-green-200 hover:opacity-90">
            <i class="fab fa-whatsapp text-xl"></i>
            Chat {{ explode(' ', $affiliator->full_name)[0] }} Sekarang
        </a>
        @endif
    </div>
</section>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
            ...
            <div class="lg:col-span-2">
                <img src="{{ url('WEB_HMTour/wp-content/uploads/2025/10/Logo-HM_UMRAH-3-WHITE.png') }}"
                ...
                <div class="flex gap-3">
                    @foreach(['instagram' => 'fab fa-instagram', 'facebook' => 'fab fa-facebook', 'youtube' => 'fab fa-youtube', 'tiktok' => 'fab fa-tiktok'] as $name => $icon)
            ...
            <div>
                ...
                <ul class="space-y-2">
                    @foreach(['Umroh Reguler', 'Umroh Plus Turki', 'Umroh Plus Dubai', 'Umroh Plus Mesir', 'Umroh Plus Al-Ula', 'Haji Plus (ONH+)'] as $item)
                    <li>
                        <a href="#paket" class="text-green-200 hover:text-white text-xs transition-colors">
                            <i class="fas fa-chevron-right text-green-400 mr-1 text-xs"></i>{{ $item }}
                        </a>
                    </li>
                    @endforeach
                </ul>
            ...
            <div>
                ...
                <ul class="space-y-2">
                    @foreach(['Tentang Kami', 'Program Kemitraan', 'Syarat & Ketentuan', 'Kebijakan Privasi', 'FAQ', 'Kontak Kami'] as $item)
                    <li>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
            <p class="text-green-200 text-xs">&copy; {{ date('Y') }} HM Tour & Travel. All rights reserved. Berizin Kemenag RI.</p>
...
<div class="wa-float">
    ...
    @else
    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $affiliator->phone_number) }}?text=Assalamu'alaikum {{ urlencode($affiliator->full_name) }}, saya ingin info paket umroh HM Tour"
       target="_blank" title="Chat {{ $affiliator->full_name }}">
        <i class="fab fa-whatsapp text-white text-2xl"></i>
    </a>
    @endif
</div>
