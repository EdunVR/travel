...
<head>
    ...
    {{-- Favicon - Dynamic from Company Settings or Default --}}
    @php
    ...
    {{-- Resolution Settings CSS --}}
    <link rel="stylesheet" href="{{ asset('css/resolution-settings.css') }}?v={{ time() }}">
    
    {{-- Desktop Responsive Scaling CSS --}}
    <link rel="stylesheet" href="{{ asset('css/desktop-responsive-scaling.css') }}?v={{ time() }}">
    ...
    {{-- Outlet Helper - Global utility untuk outlet selection --}}
    <script src="{{ asset('js/outlet-helper.js') }}?v={{ time() }}"></script>
    
    {{-- Resolution Settings - Apply user resolution preferences --}}
    <script src="{{ asset('js/resolution-settings.js') }}?v={{ time() }}"></script>
    
    {{-- Desktop Responsive Scaling - Auto-scale for desktop viewports --}}
    <script src="{{ asset('js/desktop-responsive-scaling.js') }}?v={{ time() }}"></script>
    
    {{-- Finance Components (Export/Import/Notifications) --}}
    <script src="{{ asset('js/finance-components.js') }}?v={{ time() }}"></script>
    
    {{-- Chat Panel Component --}}
    <script src="{{ asset('js/chat-panel.js') }}?v={{ time() }}"></script>

    {{-- Tab Modal Handler - Handle modals in tab system --}}
    <script src="{{ asset('js/tab-modal-handler.js') }}"></script>

    {{-- Browser Reload Redirect System --}}
    <script src="{{ asset('js/browser-reload-redirect.js') }}"></script>
    
    {{-- Iframe Auto Height & Responsive Scaling --}}
    <script src="{{ asset('js/iframe-auto-height.js') }}"></script>
    
    {{-- Responsive Scaling Helper (Debug Tool - Optional) --}}
    @if(!app()->environment('production'))
        <script src="{{ asset('js/responsive-scaling-helper.js') }}"></script>
    @endif
    
    {{-- Viewport Scale Indicator --}}
    <script src="{{ asset('js/viewport-scale-indicator.js') }}?v={{ time() }}"></script>
    
    {{-- Auto Resize Dropdown --}}
    <script src="{{ asset('js/auto-resize-dropdown.js') }}?v={{ time() }}"></script>
    ...
    {{-- CRITICAL: Install navigation blocker BEFORE Alpine.js --}}
    <script>
    (function() {
        'use strict';
    ...
    <div class="lg:pl-80 transition-all overflow-x-hidden" x-data="tabSystem()">
        ...
        <div class="relative">
            <template x-for="tab in tabs" :key="tab.id">
                ...
                >
                    {{-- Content will be loaded here via iframe or AJAX --}}
...
{{-- Emergency Fix: Prevent page reload BEFORE Alpine.js loads --}}
<script>
// ========================================
