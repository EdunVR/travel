...
<body class="bg-gradient-to-br from-green-50 to-emerald-100 min-h-screen">
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl p-8" id="registrationForm">
            ...
            <form action="{{ route('affiliate.register.store') }}" method="POST" enctype="multipart/form-data" id="affiliateForm">
                @csrf
                ...
                <div class="step-content hidden" id="step2">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="program-wrapper">
                            <input type="radio" name="partnership_program_id" value="{{ $program->id }}" 
                                   id="program-{{ $program->id }}"
                                   data-fee="{{ $program->registration_fee }}"
                                   data-commission="{{ $program->min_sale_commission }}"
                                   data-requires-booking="{{ $program->requires_previous_booking ? 'true' : 'false' }}"
                                   data-slug="{{ $program->slug }}"
                                   class="hidden program-radio" required>
                            <label for="program-{{ $program->id }}" class="block cursor-pointer">
                            <div class="program-card border-2 border-gray-200 rounded-xl p-6 hover:border-green-500 transition-all duration-200">
                                <div class="flex justify-between items-start mb-3">
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">{{ $program->target_audience }}</p>
                                        @endif
                                    ...
                                    <div class="text-right">
                                        <div class="text-2xl font-bold text-green-600">{{ $program->formatted_fee }}</div>
                                ...
                                <button type="button" 
                                        onclick="event.stopPropagation(); toggleDetails({{ $program->id }})"
                                    ...
                                    <i class="fas fa-chevron-down mr-2" id="icon-{{ $program->id }}"></i>
                                    <span id="text-{{ $program->id }}">Lihat Detail Benefit</span>
                                </button>

                                <!-- Detail Benefits (Hidden by default) -->
                                <div id="details-{{ $program->id }}" class="hidden mt-3 pt-3 border-t border-gray-200">
                                    <h4 class="text-sm font-bold text-gray-700 mb-3">
                                        <i class="fas fa-gift text-green-500 mr-2"></i>Benefit yang Anda Dapatkan:
                                    </h4>
                                    @if($program->benefits && is_array($program->benefits))
                                ...
                                <div class="flex items-center justify-between pt-3 border-t border-gray-200 mt-3">
                                    <div class="text-sm">
                                        ...
                                        <span class="font-bold text-green-600 ml-2">{{ $program->formatted_commission }}</span>
                                    </div>
                                    <div class="text-sm">
                                        <span class="text-gray-600">PPC:</span>
                                        <span class="font-bold text-blue-600 ml-2">Rp {{ number_format($program->default_ppc_commission, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                    ...
                    <div id="hierarchy-section" class="hidden mt-6 p-5 bg-blue-50 border border-blue-200 rounded-xl">
                        ...
                        <div id="dropdown-master" class="hidden mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                <i class="fas fa-building text-purple-500 mr-1"></i> Cabang
                            </label>
                            <select name="upline_master_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                                ...
                                @foreach($hmMasters as $master)
                                <option value="{{ $master->id }}" {{ old('upline_master_id') == $master->id ? 'selected' : '' }}>
                                    {{ $master->full_name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div id="dropdown-leader" class="hidden mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                <i class="fas fa-user-tie text-indigo-500 mr-1"></i> Leader
                            </label>
                            <select name="upline_leader_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                                ...
                                @foreach($hmLeaders as $leader)
                                <option value="{{ $leader->id }}" {{ old('upline_leader_id') == $leader->id ? 'selected' : '' }}>
                                    {{ $leader->full_name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div id="dropdown-partner" class="hidden mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                <i class="fas fa-handshake text-green-500 mr-1"></i> Partner
                            </label>
                            <select name="upline_partner_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                                <option value="">-- Tidak Ada --</option>
                                @foreach($hmPartners as $partner)
                                <option value="{{ $partner->id }}" {{ old('upline_partner_id') == $partner->id ? 'selected' : '' }}>
                                    {{ $partner->full_name }}
                                </option>
                                @endforeach
                            </select>
        ...
        <div class="text-center mt-6">
            <p class="text-gray-600">
                Sudah punya akun? 
                <a href="{{ route('affiliate.login') }}" class="text-green-600 hover:text-green-700 font-medium">Login di sini</a>
            </p>
        </div>
    </div>
