<x-layouts.admin :title="'Investor / Pencairan'">
      ...
      return {
        ...
        async fetchData(){
          ...
          try {
            ...
            const response = await fetch(`{{ route('admin.investor.pencairan.index') }}?${params}`);
        ...
        async submitForm(){
          ...
          try {
            const url = this.form.id 
              ? `{{ route('admin.investor.pencairan.index') }}/${this.form.id}`
              : '{{ route("admin.investor.pencairan.store") }}';
        ...
        async deleteNow(){
          ...
          this.deleting = true;
          try {
            const response = await fetch(`{{ route('admin.investor.pencairan.index') }}/${this.toDelete.id}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
        ...
        exportPdf(){
          ...
          window.open(`{{ route('admin.investor.pencairan.export') }}?${params}&format=pdf`, '_blank');
        },
        ...
        exportExcel(){
          ...
          window.open(`{{ route('admin.investor.pencairan.export') }}?${params}&format=excel`, '_blank');
        },
          ...
          setTimeout(() => {
            this.showToast = false;
                ...
                async confirmRejection() {
                    this.loading = true;
                    try {
                        const response = await fetch(`{{ route("admin.investor.pencairan.index") }}/${this.approvalId}/reject`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                ...
                async confirmPayment() {
                    this.loading = true;
                    try {
                        const response = await fetch(`{{ route("admin.investor.pencairan.index") }}/${this.approvalId}/mark-as-paid`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                ...
                async exportData() {
                    ...
                    window.open(`{{ route("admin.investor.pencairan.export") }}?${params.toString()}`);
                },
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.store('investorPencairan', {
                async viewWithdrawal(id) {
                    try {
                        const response = await fetch(`{{ route("admin.investor.pencairan.index") }}/${id}`);
                        const result = await response.json();
                ...
                async editWithdrawal(id) {
                    try {
                        const response = await fetch(`{{ route("admin.investor.pencairan.index") }}/${id}/edit`);
                        const result = await response.json();
