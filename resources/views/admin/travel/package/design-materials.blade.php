<div x-data="designMaterials({{ $packageId }})" x-init="init()" class="space-y-6">
    <!-- Header -->
...
function designMaterials(packageId) {
    return {
        ...
        async fetchMaterials() {
            try {
                const response = await fetch(`{{ url(route('admin.inventaris.travel.design-materials.data', ['packageId' => '__ID__'])) }}`.replace('__ID__', this.packageId));
        ...
        async submitUpload() {
            ...
            try {
                ...
                console.log('Uploading to:', `{{ url(route('admin.inventaris.travel.design-materials.upload', ['packageId' => '__ID__'])) }}`.replace('__ID__', this.packageId));
                
                const response = await fetch(`{{ url(route('admin.inventaris.travel.design-materials.upload', ['packageId' => '__ID__'])) }}`.replace('__ID__', this.packageId), {
                    method: 'POST',
                    headers: {
        ...
        async toggleComplete(material) {
            try {
                ...
                const baseUrl = `{{ url('/admin/inventaris/travel/materials') }}`;
        ...
        async deleteMaterial(materialId) {
            ...
            try {
                const response = await fetch(`{{ url(route('admin.inventaris.travel.design-materials.destroy', ['id' => '__ID__'])) }}`.replace('__ID__', materialId), {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Content-Type': 'application/json'
        ...
        downloadMaterial(materialId) {
            window.location.href = `{{ url(route('admin.inventaris.travel.design-materials.download', ['id' => '__ID__'])) }}`.replace('__ID__', materialId);
        },
