<x-layouts.admin :title="'Travel / Tambah Paket Perjalanan'">
  ...
  <script>
    function packageCreate() {
      return {
        ...
        async loadFlights() {
          try {
            const response = await fetch('{{ route("admin.inventaris.travel.package.hpp.flights") }}');
        ...
        async submitForm() {
          ...
          try {
            ...
            const response = await fetch('{{ route('admin.inventaris.travel.package.store') }}', {
              method: 'POST',
              headers: {
