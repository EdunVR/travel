<div x-data="workflowManager({{ $packageId }})" x-init="init()" class="space-y-6">
...
function workflowManager(packageId) {
    return {
        ...
        async fetchWorkflowData() {
            try {
                const response = await fetch(`{{ url(route('admin.inventaris.travel.package.workflow.progress', ['id' => '__ID__'])) }}`.replace('__ID__', this.packageId));
        ...
        async fetchTasks() {
            try {
                const response = await fetch(`{{ url(route('admin.inventaris.travel.package.workflow.tasks', ['id' => '__ID__'])) }}`.replace('__ID__', this.packageId));
        ...
        async transitionWorkflow() {
            ...
            try {
                const response = await fetch(`{{ url(route('admin.inventaris.travel.package.workflow.transition', ['id' => '__ID__'])) }}`.replace('__ID__', this.packageId), {
                    method: 'POST',
                    headers: {
