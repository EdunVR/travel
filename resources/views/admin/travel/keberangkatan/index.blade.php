<x-layouts.admin :title="'Travel / Keberangkatan'">
        ...
        <template x-for="k in keberangkatans" :key="k.id">
          <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-card">
            ...
            <div class="mt-3 flex gap-2 flex-wrap">
              ...
              @hasPermission('travel.keberangkatan.delete')
      ...
      return {
        ...
        async fetchData(){
          ...
          try {
            ...
            const response = await fetch(`{{ route('admin.inventaris.travel.keberangkatan.data') }}?${params}`);
        ...
        async fetchPackages(){
          try {
            const response = await fetch('{{ route("admin.inventaris.travel.keberangkatan.packages") }}');
            const data = await response.json();
            this.packages = data;
          } catch (error) {
        ...
        async generateKeberangkatanCode() {
          ...
          try {
            const response = await fetch('{{ route("admin.inventaris.travel.keberangkatan.generate-code") }}', {
              method: 'POST',
              headers: {
        ...
        async openEdit(k){ 
          try {
            const response = await fetch(`{{ route('admin.inventaris.travel.keberangkatan.show', '') }}/${k.id}`, {
              headers: {
        ...
        async showDetail(k){
          window.location.href = `{{ route('admin.inventaris.travel.keberangkatan.show', '') }}/${k.id}`;
        },
        ...
        async submitForm(){
          ...
          try {
            const url = this.form.id 
              ? `{{ route('admin.inventaris.travel.keberangkatan.update', '') }}/${this.form.id}`
              : '{{ route("admin.inventaris.travel.keberangkatan.store") }}';
        ...
        async deleteNow(){
          ...
          this.deleting = true;
          try {
            const response = await fetch(`{{ route('admin.inventaris.travel.keberangkatan.destroy', '') }}/${this.toDelete.id}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
