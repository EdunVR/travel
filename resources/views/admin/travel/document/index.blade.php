...
@section('title', 'Document Management - ' . $booking->jamaah->nama_lengkap)
        ...
        <div class="row mb-2">
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('admin.inventaris.booking.index') }}">Bookings</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('admin.inventaris.booking.show', $booking->id) }}">{{ $booking->booking_code }}</a></li>
    ...
    <div class="container-fluid">
        ...
        <div class="card card-primary">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <p><strong>Name:</strong> {{ $booking->jamaah->nama_lengkap }}</p>
                        <p><strong>Booking Code:</strong> {{ $booking->booking_code }}</p>
                        <p><strong>Package:</strong> {{ $booking->travelPackage->package_name }}</p>
                    </div>
                    <div class="col-md-6">
                        <p><strong>Departure:</strong> {{ $booking->keberangkatan ? $booking->keberangkatan->keberangkatan_name : 'Not assigned' }}</p>
                        <p><strong>Departure Date:</strong> {{ $booking->keberangkatan ? $booking->keberangkatan->departure_date->format('d M Y') : '-' }}</p>
                    </div>
                </div>
            </div>
        </div>
                                ...
                                <div class="card-body">
                                    <div class="d-flex justify-content-between align-items-start">
                                        <div>
                                            <h5 class="mb-1">
                                                @if($document)
                                                    <i class="fas fa-check-circle text-{{ $statusColor }}"></i>
                                                @else
                                            ...
                                            @if($document)
                                                @if($document->document_number)
                                                    <p class="mb-0 mt-2"><small><strong>Number:</strong> {{ $document->document_number }}</small></p>
                                                @endif
                                                @if($document->expiry_date)
                                                    <p class="mb-0"><small><strong>Expiry:</strong> {{ $document->expiry_date->format('d M Y') }}</small></p>
                                                    @if($isExpired)
                                                ...
                                                @if($document->verified_at)
                                                    <p class="mb-0"><small><strong>Verified by:</strong> {{ $document->verifier->name ?? 'N/A' }}</small></p>
                                                    <p class="mb-0"><small><strong>Verified at:</strong> {{ $document->verified_at->format('d M Y H:i') }}</small></p>
                                                @endif
                                            @endif
                                        </div>
                                        <div class="btn-group-vertical">
                                            <button type="button" class="btn btn-sm btn-primary mb-1" onclick="openUploadModal('{{ $docType }}', {{ $document ? $document->id : 'null' }})">
                                                <i class="fas fa-upload"></i> {{ $document ? 'Replace' : 'Upload' }}
                                            </button>
                                            @if($document)
                                                <button type="button" class="btn btn-sm btn-info mb-1" onclick="previewDocument({{ $document->id }})">
                                                    <i class="fas fa-eye"></i> Preview
                                                </button>
                                                <button type="button" class="btn btn-sm btn-success mb-1" onclick="downloadDocument({{ $document->id }})">
                                                    <i class="fas fa-download"></i> Download
                                                </button>
                                                @if($document->status !== 'approved')
                                                    <button type="button" class="btn btn-sm btn-warning mb-1" onclick="openVerifyModal({{ $document->id }}, 'approved')">
                                                        <i class="fas fa-check"></i> Approve
                                                    </button>
                                                @endif
                                                @if($document->status !== 'rejected')
                                                    <button type="button" class="btn btn-sm btn-danger mb-1" onclick="openVerifyModal({{ $document->id }}, 'rejected')">
                                    ...
                                    @if($document && $document->notes)
                                        <div class="mt-2">
                                            <small><strong>Notes:</strong> {{ $document->notes }}</small>
                                        </div>
                                    @endif
                                </div>
                            </div>
