<x-layouts.admin :title="'Pengingat Pembayaran'">
...
function paymentReminderPage() {
    return {
        ...
        async loadData() {
            try {
                const res = await fetch('{{ route("admin.inventaris.travel.payment-reminder.data") }}');
        ...
        async saveSettings() {
            ...
            try {
                const res = await fetch('{{ route("admin.inventaris.travel.payment-reminder.settings") }}', {
                    method: 'POST',
        ...
        async triggerManual(dryRun) {
            ...
            try {
                const res = await fetch('{{ route("admin.inventaris.travel.payment-reminder.trigger") }}', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content },
