<x-layouts.admin :title="'Travel / Detail Booking'">
<style>
  ...
  <div class="container-fluid">
    ...
    <div class="row mb-3">
      <div class="col-12">
        <div class="d-flex justify-content-between align-items-center">
          ...
          <div>
            <a href="{{ route('admin.inventaris.booking.index') }}" class="btn btn-secondary">
    ...
    <div class="row">
      ...
      <div class="col-md-8">
            ...
            <div class="row">
              <div class="col-md-6">
                <table class="table table-sm table-borderless">
                  ...
                  <tr>
                    <th>Outlet</th>
                    <td>{{ $booking->outlet->nama_outlet ?? '-' }}</td>
                  </tr>
                </table>
              </div>
              <div class="col-md-6">
                <table class="table table-sm table-borderless">
                  <tr>
                    ...
                    <td>
                      <a href="{{ route('admin.inventaris.travel.package.detail', $booking->id_travel_package) }}">
                  ...
                  <tr>
                    <th>Keberangkatan</th>
                    <td>
                      @if($booking->keberangkatan)
                        <a href="{{ route('admin.inventaris.travel.keberangkatan.show', $booking->id_keberangkatan) }}">
                          {{ $booking->keberangkatan->keberangkatan_name }}
                        </a>
                      @else
                  ...
                  <tr>
                    ...
                    <td>
                      @if($booking->keberangkatan)
                        {{ $booking->keberangkatan->departure_date->format('d M Y') }}
                      @else
                        {{ $booking->travelPackage->departure_date->format('d M Y') }}
                      @endif
                    </td>
                  </tr>
                </table>
              ...
              <div class="col-md-6">
                <table class="table table-sm table-borderless">
                  ...
                  <tr>
                    ...
                    <td>
                      @if($booking->jamaah->passport_tanggal_kadaluarsa)
                        {{ \Carbon\Carbon::parse($booking->jamaah->passport_tanggal_kadaluarsa)->format('d M Y') }}
                      @else
                        -
                      @endif
                    </td>
                  ...
                  <tr>
                    <th>Mahram</th>
                    <td>{{ $booking->jamaah->mahram_name ?? '-' }}</td>
                  </tr>
                  <tr>
                    <th>Kontak Darurat</th>
                    <td>
                      @if($booking->jamaah->emergency_contact_name)
                        {{ $booking->jamaah->emergency_contact_name }} 
                        ({{ $booking->jamaah->emergency_contact_phone }})
                      @else
                        -
                      @endif
                    </td>
        ...
        <div class="card mb-3" x-data="jamaahHotelManager({{ $booking->id }})">
          <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0"><i class="fas fa-hotel mr-2"></i>Booking Hotel Jamaah</h5>
            @if($booking->status !== 'cancelled')
            <a href="{{ route('admin.inventaris.booking.hotel-bookings.create', $booking->id) }}" 
                ...
                <tbody>
                  <template x-for="hb in hotels" :key="hb.id">
                    <tr>
                      ...
                      <td class="text-right">
                        <a href="{{ route('admin.inventaris.booking.hotel-bookings.edit', [$booking->id, ':id']) }}" 
        ...
        <!-- Add-ons Section -->
        <div class="card mb-3" id="addons-section" x-data="addonManager({{ $booking->id }})">
          <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Add-ons / Request Tambahan</h5>
            @if($booking->status !== 'cancelled')
            <a href="{{ route('admin.inventaris.booking.addons.create', $booking->id) }}" 
               class="btn btn-sm btn-primary"
               x-data="{ loading: false }"
               @click="loading = true"
               :disabled="loading">
                ...
                <tbody>
                  <template x-for="addon in addons" :key="addon.id">
                    <tr>
                      ...
                      <td class="text-right">
                        <a href="{{ route('admin.inventaris.booking.addons.edit', [$booking->id, ':id']) }}" 
                           class="btn btn-xs btn-outline-primary mr-1"
          ...
          <div class="card-body">
            @if($booking->payments->count() > 0)
              ...
              <div class="row mb-3">
                    ...
                    <div class="card-body py-3">
                      <div class="d-flex justify-content-between align-items-center">
                        <div>
                          ...
                          <h4 class="mb-0">Rp {{ number_format($booking->paid_amount ?? 0, 0, ',', '.') }}</h4>
                        </div>
                ...
                <div class="col-md-4">
                  <div class="card bg-{{ $booking->remaining_amount > 0 ? 'warning' : 'success' }} text-white">
                    <div class="card-body py-3">
                      <div class="d-flex justify-content-between align-items-center">
                        <div>
                          <h6 class="mb-1">Sisa Tagihan</h6>
                          <h4 class="mb-0">Rp {{ number_format($booking->remaining_amount ?? 0, 0, ',', '.') }}</h4>
                        </div>
                        <i class="fas fa-{{ $booking->remaining_amount > 0 ? 'exclamation-circle' : 'check-double' }} fa-2x opacity-50"></i>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <!-- Payment List -->
              <h6 class="mb-3"><i class="fas fa-list mr-2"></i>Detail Pembayaran ({{ $booking->payments->count() }} transaksi)</h6>
              @foreach($booking->payments->sortByDesc('payment_date') as $index => $payment)
                    ...
                    <div class="col-md-8">
                      <div class="d-flex justify-content-between align-items-start mb-2">
                        <div>
                          ...
                          <small class="text-muted">
                            <i class="fas fa-calendar mr-1"></i>{{ $payment->payment_date->format('d M Y') }}
                            <i class="fas fa-clock ml-2 mr-1"></i>{{ $payment->created_at->format('H:i') }}
                          </small>
                        </div>
                        <span class="badge badge-{{ $payment->payment_type === 'full' ? 'success' : 'warning' }} badge-pill">
                      ...
                      <div class="row small">
                        <div class="col-6">
                          <p class="mb-1">
                            ...
                            <strong>Metode:</strong> {{ $payment->formatted_payment_method }}
                          </p>
                        ...
                        <div class="col-6">
                          ...
                          <p class="mb-0 text-muted">
                            ...
                            <small>{{ $payment->recordedBy ? $payment->recordedBy->name : 'Website (Public)' }}</small>
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="col-md-4 text-right">
                      ...
                      <div class="btn-group-vertical btn-group-sm" role="group">
                        <button type="button" 
                                ...
                                data-url="{{ route('admin.inventaris.payment.receipt', $payment->id) }}">
                          <i class="fas fa-file-pdf"></i> Kwitansi
                        </button>
                        <a href="{{ route('admin.inventaris.payment.receipt.download', $payment->id) }}" 
                           class="btn btn-primary">
                          <i class="fas fa-download"></i> Download
                        </a>
                        @hasPermission('travel.payment.delete')
                        <button type="button"
        ...
        <div class="card mb-3">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div x-data="{ loadingManage: false, loadingUpload: false }">
              <a href="{{ route('admin.inventaris.document.index', $booking->id) }}" 
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      @endphp
                      <span class="badge badge-{{ $docColor }}">{{ ucfirst($doc->status) }}</span>
                    </td>
                    <td>
                      @if($doc->file_path)
      ...
      <div class="col-md-4">
        <div class="card mb-3">
          ...
          <div class="card-body">
            ...
            @endphp
            <table class="table table-sm table-borderless">
              @if(count($showFamilyMembers) > 0)
              <tr>
                <th>Paket Utama ({{ $showMainPax }} Pax)</th>
                <td class="text-right">Rp {{ number_format($showUnitPrice * $showMainPax, 0, ',', '.') }}</td>
              </tr>
              @if($showFamilyDiscountTotal > 0)
              <tr>
                <th>Anggota Keluarga (Diskon)</th>
                <td class="text-right">Rp {{ number_format($showFamilyDiscountTotal, 0, ',', '.') }}</td>
              </tr>
              @endif
              @else
              <tr>
                <th>Harga Paket</th>
                <td class="text-right">Rp {{ number_format($showUnitPrice, 0, ',', '.') }}</td>
              </tr>
              @endif
              ...
              @if($showHotelCharge > 0)
              <tr>
                <th>Hotel (Charge)</th>
                <td class="text-right text-warning">Rp {{ number_format($showHotelCharge, 0, ',', '.') }}</td>
              </tr>
              @endif

              @if($showAddonsTotal > 0)
              <tr>
                <th>Add-ons / Request</th>
                <td class="text-right text-info">Rp {{ number_format($showAddonsTotal, 0, ',', '.') }}</td>
              </tr>
              @endif
              ...
              <tr class="border-top">
                <th>Total Harga</th>
                <td class="text-right">Rp {{ number_format($showGrandTotal + $booking->discount_amount, 0, ',', '.') }}</td>
              </tr>
              ...
              <tr class="text-success">
                <th>Diskon Voucher ({{ $booking->voucher_code }})</th>
                <td class="text-right">- Rp {{ number_format($booking->voucher_discount, 0, ',', '.') }}</td>
              </tr>
              @endif
              
              @if($booking->hasAdminDiscount())
              <tr class="text-success">
                <th>Diskon Admin</th>
                <td class="text-right">- Rp {{ number_format($booking->admin_discount, 0, ',', '.') }}</td>
              </tr>
              @endif
              
              @if($booking->hasVoucher() || $booking->hasAdminDiscount())
              <tr class="border-top">
                <th>Total Setelah Diskon</th>
                <td class="text-right"><strong>Rp {{ number_format($booking->getFinalTotal(), 0, ',', '.') }}</strong></td>
              </tr>
              @else
              <tr>
                <th>Total Bayar</th>
                <td class="text-right"><strong>Rp {{ number_format($showGrandTotal, 0, ',', '.') }}</strong></td>
              </tr>
              @endif
              
              <tr>
                <th>Sudah Dibayar</th>
                <td class="text-right text-success">Rp {{ number_format($booking->paid_amount, 0, ',', '.') }}</td>
              </tr>
              <tr class="border-top">
                <th>Sisa Pembayaran</th>
                <td class="text-right text-danger">
                  <strong>Rp {{ number_format($booking->getRemainingBalanceAfterDiscounts(), 0, ',', '.') }}</strong>
                </td>
              </tr>
              <tr>
                ...
                <td class="text-right">
                  ...
                  @endphp
                  <span class="badge badge-{{ $paymentColor }}">{{ ucfirst($booking->payment_status) }}</span>
                </td>
              </tr>
            </table>

            @if($booking->id_invoice && $booking->invoice)
            <div class="alert alert-info mt-3">
              <div class="d-flex justify-content-between align-items-start">
                <div class="flex-grow-1">
                  <i class="fas fa-file-invoice"></i>
                  <strong>Invoice:</strong> {{ $booking->invoice->no_invoice ?? 'N/A' }}
                  <br>
                  <small class="d-block mt-2">
                    ...
                    @endphp
                    <span class="badge badge-{{ $invoiceColor }}">
                ...
                <div class="btn-group-vertical btn-group-sm">
                  <a href="javascript:void(0)" 
                     onclick="window.open('{{ route('admin.inventaris.payment.jamaah-invoice-pdf', $booking->id) }}', '_blank')"
                     class="btn btn-sm btn-primary mb-1">
                    <i class="fas fa-file-pdf"></i> Lihat PDF
                  </a>
                  <a href="javascript:void(0)" 
                     onclick="window.open('{{ route('admin.inventaris.payment.jamaah-invoice-pdf', ['booking' => $booking->id, 'download' => 1]) }}', '_blank')"
        ...
        <div class="card">
          ...
          <div class="card-body">
            <table class="table table-sm table-borderless">
              <tr>
                <th>Tipe</th>
                <td class="text-right">{{ ucfirst($booking->travelPackage->package_type) }}</td>
              </tr>
              <tr>
                <th>Durasi</th>
                <td class="text-right">{{ $booking->travelPackage->duration_days }} hari</td>
              </tr>
              <tr>
                <th>Keberangkatan</th>
                <td class="text-right">{{ $booking->travelPackage->departure_date->format('d M Y') }}</td>
              </tr>
              <tr>
                <th>Kepulangan</th>
                <td class="text-right">{{ $booking->travelPackage->return_date->format('d M Y') }}</td>
              </tr>
              @if($booking->travelPackage->flightDeparture)
              <tr>
                <th>Maskapai Berangkat</th>
                <td class="text-right">
                  {{ $booking->travelPackage->flightDeparture->airline_name }}
                  ({{ $booking->travelPackage->flightDeparture->flight_number }})
                  <br><small class="text-muted">{{ $booking->travelPackage->flightDeparture->departure_airport }} → {{ $booking->travelPackage->flightDeparture->arrival_airport }}</small>
                </td>
              </tr>
              @endif
              ...
              <tr>
                ...
                <td class="text-right">
                  {{ $booking->travelPackage->flightReturn->airline_name }}
                  ({{ $booking->travelPackage->flightReturn->flight_number }})
                  <br><small class="text-muted">{{ $booking->travelPackage->flightReturn->departure_airport }} → {{ $booking->travelPackage->flightReturn->arrival_airport }}</small>
                </td>
              </tr>
              @endif
              @php
              ...
              <tr>
                <th>Hotel Mekkah</th>
                <td class="text-right">
                  @if($jamaahHotelMakkah)
                    {{ $jamaahHotelMakkah->hotel->hotel_name ?? '-' }}
                    @if($jamaahHotelMakkah->room_type) <small>({{ ucfirst($jamaahHotelMakkah->room_type) }})</small>@endif
                    @if($jamaahHotelMakkah->is_charged)
                      <span class="badge badge-warning badge-sm">Charge</span>
                    @else
                      <span class="badge badge-success badge-sm">Include</span>
                    @endif
                  @elseif($pkgHotelMakkah)
                    {{ $pkgHotelMakkah->hotel_name }}
                    @if($pkgHotelMakkah->star_rating) <small>({{ $pkgHotelMakkah->star_rating }}⭐)</small>@endif
                    <span class="badge badge-success badge-sm">Include</span>
                  @else
              ...
              <tr>
                <th>Hotel Madinah</th>
                <td class="text-right">
                  @if($jamaahHotelMadinah)
                    {{ $jamaahHotelMadinah->hotel->hotel_name ?? '-' }}
                    @if($jamaahHotelMadinah->room_type) <small>({{ ucfirst($jamaahHotelMadinah->room_type) }})</small>@endif
                    @if($jamaahHotelMadinah->is_charged)
                      <span class="badge badge-warning badge-sm">Charge</span>
                    @else
                      <span class="badge badge-success badge-sm">Include</span>
                    @endif
                  @elseif($pkgHotelMadinah)
                    {{ $pkgHotelMadinah->hotel_name }}
                    @if($pkgHotelMadinah->star_rating) <small>({{ $pkgHotelMadinah->star_rating }}⭐)</small>@endif
                    <span class="badge badge-success badge-sm">Include</span>
                  @else
                    <span class="text-muted">-</span>
                  @endif
              ...
              <tr>
                <th>Harga Paket</th>
                <td class="text-right">Rp {{ number_format($booking->travelPackage->price, 0, ',', '.') }}</td>
              </tr>
            </table>
          </div>
        </div>
    ...
    <div class="modal-dialog modal-dialog-scrollable" style="margin-top: 2rem;">
      <div class="modal-content">
        ...
        <form id="add-communication-form">
          <div class="modal-body">
            <input type="hidden" name="id_member" value="{{ $booking->id_member }}">
            <input type="hidden" name="id_travel_package" value="{{ $booking->id_travel_package }}">
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $('#btn-create-invoice').on('click', function() {
        ...
        // Set default values
        $('#seller_name').val('{{ auth()->user()->name }}');
        $('#discount_amount').val('{{ $booking->discount_amount ?? 0 }}');
        $('#closing_source').val('{{ $booking->closing_source ?? "kantor" }}');
      ...
      $('#btn-edit-invoice').on('click', function() {
        ...
        $('#seller_name').val('{{ $booking->seller_name ?? auth()->user()->name }}');
        $('#discount_amount').val('{{ $booking->discount_amount ?? 0 }}');
        $('#closing_source').val('{{ $booking->closing_source ?? "kantor" }}');
        $('#terms_conditions').val(`{{ $booking->terms_conditions ?? '' }}`);
      ...
      $('#payment-form').on('submit', function(e) {
        ...
        $.ajax({
          url: '{{ route("admin.inventaris.payment.store", $booking->id) }}',
          method: 'POST',
          data: formData,
          beforeSend: function() {
      ...
      function loadInvoicePreview() {
        ...
        const previewUrl = '{{ route("admin.inventaris.payment.preview-invoice", $booking->id) }}' + 
      ...
      $('#btn-save-invoice').on('click', function() {
        ...
        const url = isEditMode 
          ? '{{ route("admin.inventaris.payment.update-invoice", $booking->id) }}'
          : '{{ route("admin.inventaris.payment.create-invoice", $booking->id) }}';
      ...
      $('#upload-document-form').on('submit', function(e) {
        ...
        $.ajax({
          url: '{{ route("admin.inventaris.document.upload", $booking->id) }}',
          method: 'POST',
          data: formData,
          processData: false,
          contentType: false,
      ...
      $(document).on('click', '#btn-delete-invoice', function() {
        const paymentCount = {{ $booking->payments->count() }};
      ...
      function doDeleteInvoice() {
        $.ajax({
          ...
          method: 'DELETE',
          data: { _token: '{{ csrf_token() }}', force: 1 },
          success: function(response) {
            if (response.success) {
              if (typeof Swal !== 'undefined') {
      ...
      function doDeletePayment(paymentId) {
        $.ajax({
          url: '{{ url("admin/inventaris/travel/payment") }}/' + paymentId,
          method: 'DELETE',
    ...
    function jamaahHotelManager(bookingId) {
      return {
        ...
        async fetchAvailableHotels() {
          try {
            const res = await fetch(`{{ route('admin.inventaris.hotel.data') }}`);
            const data = await res.json();
        ...
        async onHotelChange() {
          ...
          try {
            const res = await fetch(`{{ url('admin/inventaris/hotel') }}/${this.hotelForm.id_hotel}/room-types`);
            const data = await res.json();
        ...
        async openEdit(hb) {
          ...
          if (hb.id_hotel) {
            try {
              const res = await fetch(`{{ url('admin/inventaris/hotel') }}/${hb.id_hotel}/room-types`);
              const data = await res.json();
              this.availableRoomTypes = data.data || [];
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <input type="number" class="form-control" id="amount" name="amount" 
                     min="0" max="{{ $showSisa }}" step="0.01" required>
              <small class="form-text text-muted">
                Maksimal: Rp {{ number_format($showSisa, 0, ',', '.') }}
              </small>
            </div>
                  ...
                  <div class="card-body py-2">
                    <div class="form-group mb-2">
                      ...
                      <input type="text" class="form-control form-control-sm" id="seller_name" name="seller_name" 
                             value="{{ auth()->user()->name }}" placeholder="Nama seller/closing">
                    ...
                    <div class="form-group mb-0">
                      ...
                      <select class="form-control form-control-sm" id="closing_source" name="closing_source">
                        <option value="kantor" {{ ($booking->closing_source ?? '') == 'kantor' ? 'selected' : '' }}>Kantor</option>
                        <option value="alumni" {{ ($booking->closing_source ?? '') == 'alumni' ? 'selected' : '' }}>Alumni</option>
                        <option value="digital_marketing" {{ ($booking->closing_source ?? '') == 'digital_marketing' ? 'selected' : '' }}>Digital Marketing</option>
                        <option value="event" {{ ($booking->closing_source ?? '') == 'event' ? 'selected' : '' }}>Event</option>
                      </select>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="card-body py-2">
                    <div class="form-group mb-0">
                      ...
                      <input type="number" class="form-control form-control-sm" id="discount_amount" name="discount_amount" 
                             value="0" min="0" max="{{ $booking->total_price }}" step="1000">
