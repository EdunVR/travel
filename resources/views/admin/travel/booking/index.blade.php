<x-layouts.admin :title="'Travel / Booking Jamaah'">
  <div x-data="bookingCrud()" x-init="init()" class="space-y-4 overflow-x-hidden self-start w-full">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        <a href="{{ route('admin.inventaris.travel.payment.verify') }}" class="inline-flex items-center gap-2 rounded-xl bg-amber-600 text-white px-4 py-2 hover:bg-amber-700 transition-colors relative">
        ...
        @hasPermission('travel.booking.create')
              ...
              <tr class="hover:bg-slate-50">
                ...
                <td class="px-2 py-2">
                  <div class="flex gap-0.5 justify-end">
                    ...
                    @hasPermission('travel.booking.update')
                    ...
                    @endhasPermission
                    @hasPermission('travel.booking.delete')
      ...
      return {
        ...
        async fetchData() {
          ...
          try {
            ...
            const response = await fetch(`{{ route('admin.inventaris.booking.getData') }}?${params}`);
        ...
        async fetchOutlets() {
          try {
            const response = await fetch('{{ route("admin.inventaris.outlet.data") }}');
            const data = await response.json();
        ...
        async onOutletChange() {
          ...
          try {
            const [packagesRes, membersRes] = await Promise.all([
              fetch(`{{ route('admin.inventaris.booking.getAvailablePackages') }}?outlet_id=${this.form.id_outlet}`),
              fetch(`{{ route('admin.inventaris.booking.getJamaahMembers') }}?outlet_id=${this.form.id_outlet}`)
            ]);
        ...
        async onPackageChange() {
          ...
          try {
            const response = await fetch(`{{ url('admin/inventaris/travel/booking/keberangkatan') }}/${this.form.id_travel_package}`);
        ...
        async editBooking(id) {
          try {
            const response = await fetch(`{{ url('admin/inventaris/travel/booking') }}/${id}/edit`);
        ...
        viewBooking(id) {
          window.location.href = `{{ url('admin/inventaris/travel/booking') }}/${id}`;
        },
        ...
        async submitForm() {
          ...
          try {
            const url = this.form.id 
              ? `{{ url('admin/inventaris/travel/booking') }}/${this.form.id}`
              : '{{ route("admin.inventaris.booking.store") }}';
        ...
        async cancelBooking() {
          ...
          try {
            const response = await fetch(`{{ url('admin/inventaris/travel/booking') }}/${this.toCancel.id}/cancel`, {
              method: 'POST',
              headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
        ...
        async submitPaymentAmount() {
          ...
          try {
            const response = await fetch(`{{ url('admin/inventaris/travel/booking') }}/${this.selectedBookingForPayment.id}/set-payment-amount`, {
              method: 'POST',
              headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
        ...
        async sendPaymentLink(booking) {
          ...
          try {
            // Build payment link
            const paymentLink = `{{ url('/') }}/paket/${booking.id_travel_package}/invoice/${booking.id}`;
        ...
        async deleteBooking(booking) {
          this.deleting = true;
          try {
            const response = await fetch(`{{ url('admin/inventaris/travel/booking') }}/${booking.id}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
