    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-4 flex flex-col"
        ...
        <div class="flex-1 overflow-y-auto">
            <form @submit.prevent="submitApproval()" class="p-6 space-y-6">
                ...
                {{-- Fixed Asset Fields (for Fixed Asset) --}}
                ...
                {{-- Journal Entry Fields (for Journal) --}}
                ...
                {{-- Book Selection (for Fixed Asset) - Moved above --}}
    ...
    return {
        ...
        async loadSuppliers() {
            try {
                ...
                const url = new URL('{{ route("admin.supply-chain.permintaan-barang.suppliers") }}');
        ...
        async loadBooks() {
            try {
                // Get outlet_id from selected item
                const outletId = this.selectedItem?.outlet_id || this.selectedItem?.outlet?.id;
                const url = new URL('{{ route("admin.supply-chain.permintaan-barang.books") }}');
        ...
        async loadAssetAccounts() {
            try {
                // Get outlet_id from selected item
                const outletId = this.selectedItem?.outlet_id || this.selectedItem?.outlet?.id;
                const url = new URL('{{ route("admin.supply-chain.permintaan-barang.asset-accounts") }}');
        ...
        async loadExpenseAccounts() {
            try {
                const outletId = this.selectedItem?.outlet_id || this.selectedItem?.outlet?.id;
                const url = new URL('{{ route("admin.supply-chain.permintaan-barang.expense-accounts") }}');
        ...
        async loadAccumulatedDepreciationAccounts() {
            try {
                const outletId = this.selectedItem?.outlet_id || this.selectedItem?.outlet?.id;
                const url = new URL('{{ route("admin.supply-chain.permintaan-barang.accumulated-depreciation-accounts") }}');
        ...
        async loadPaymentAccounts() {
            try {
                const outletId = this.selectedItem?.outlet_id || this.selectedItem?.outlet?.id;
                const url = new URL('{{ route("admin.supply-chain.permintaan-barang.payment-accounts") }}');
        ...
        async loadJournalAccounts() {
            try {
                const outletId = this.selectedItem?.outlet_id || this.selectedItem?.outlet?.id;
                const url = new URL('{{ route("admin.supply-chain.permintaan-barang.journal-accounts") }}');
        ...
        async submitApproval() {
            ...
            try {
                ...
                const response = await fetch(`{{ route('admin.supply-chain.permintaan-barang.approve', ':id') }}`.replace(':id', this.selectedItem.id), {
                    method: 'POST',
                    headers: {
