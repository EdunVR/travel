...
<x-layouts.admin :title="'Aktiva Tetap'">
  <div x-data="fixedAssetsManagement()" x-init="init()" class="space-y-6">
    ...
    {{-- Modal Book Selection for Depreciation Posting --}}
      ...
      return {
        ...
        async loadAllAssets() {
          try {
            ...
            const response = await fetch(`{{ route("finance.fixed-assets.assets.all") }}?${params.toString()}`, {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
        ...
        async loadAccounts() {
          try {
            ...
            const [assetResult, expenseResult, contraResult, paymentResult] = await Promise.all([
              fetch(`{{ route("finance.accounts.active") }}?outlet_id=${outletId}&type=asset&exclude_contra=true`, { method: 'GET', headers }).then(r => r.json()),
              fetch(`{{ route("finance.accounts.active") }}?outlet_id=${outletId}&type=expense`, { method: 'GET', headers }).then(r => r.json()),
              fetch(`{{ route("finance.accounts.active") }}?outlet_id=${outletId}&type=asset&contra=true`, { method: 'GET', headers }).then(r => r.json()),
              fetch(`{{ route("finance.accounts.active") }}?outlet_id=${outletId}&type=asset&exclude_contra=true`, { method: 'GET', headers }).then(r => r.json())
            ]);
        ...
        async approveAsset(id) {
          ...
          try {
            const response = await fetch(`{{ route("finance.fixed-assets.approve", ":id") }}`.replace(':id', id), {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        async executeBulkPostDepreciations(bookId) {
          ...
          try {
            const response = await fetch('{{ route("finance.fixed-assets.depreciation.bulk-post") }}', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        async bulkDeleteDepreciations() {
          ...
          try {
            const response = await fetch('{{ route("finance.fixed-assets.depreciation.bulk-delete") }}', {
              method: 'DELETE',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
