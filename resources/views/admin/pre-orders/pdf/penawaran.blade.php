...
<head>
    <meta charset="utf-8">
    <title>Penawaran {{ $preorder->kode_preorder }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="client-info">
        <table>
            <tr>
                ...
                <td style="width: 2%;">:</td>
                <td style="width: 33%;">{{ $preorder->customer->nama ?? '-' }}</td>
                <td style="width: 15%;">Tgl Penawaran</td>
                <td style="width: 2%;">:</td>
                <td>{{ $preorder->tanggal->format('d/m/Y') }}</td>
            </tr>
            <tr>
                <td>Up</td>
                <td>:</td>
                <td>{{ $preorder->customer->nama_cp ?? '-' }}</td>
                <td>Alamat</td>
                <td>:</td>
                <td>{{ $preorder->customer->alamat ?? '-' }}</td>
            </tr>
            <tr>
                <td></td>
                <td></td>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <img src="{{ $item->product_image }}" alt="{{ $item->deskripsi }}" class="product-image"><br>
                    @endif
                    {{ $item->deskripsi }}
                    ...
                    <br>• Material Instalasi: Rp {{ number_format($item->material_instalasi_biaya, 0, ',', '.') }} / {{ $item->material_instalasi_satuan }}
                    @if($item->material_instalasi_keterangan)
                    <br>&nbsp;&nbsp;<em>{{ $item->material_instalasi_keterangan }}</em>
                    @endif
                    @endif
                    
                    @if($item->pemasangan_pelatihan_biaya > 0)
                    <br>• Pemasangan & Pelatihan: Rp {{ number_format($item->pemasangan_pelatihan_biaya, 0, ',', '.') }} / {{ $item->pemasangan_pelatihan_satuan }}
                    @if($item->pemasangan_pelatihan_keterangan)
                    <br>&nbsp;&nbsp;<em>{{ $item->pemasangan_pelatihan_keterangan }}</em>
                    @endif
                    @endif
                    
                    @if($item->ongkos_kirim_biaya > 0)
                    <br>• Ongkos Kirim: Rp {{ number_format($item->ongkos_kirim_biaya, 0, ',', '.') }} / {{ $item->ongkos_kirim_satuan }}
                    ...
                    <br><strong>Total Biaya Tambahan: Rp {{ number_format($item->calculateTotalBiayaTambahan(), 0, ',', '.') }}</strong>
                    @endif
                </td>
                ...
                <td class="text-right">
                    ...
                    <br>+ Rp {{ number_format($item->calculateTotalBiayaTambahan(), 0, ',', '.') }}
                    <br><strong>= Rp {{ number_format($item->total_with_additional_costs, 0, ',', '.') }}</strong>
                    @endif
                </td>
                <td>
                    @if($item->material_instalasi_biaya > 0 || $item->pemasangan_pelatihan_biaya > 0 || $item->ongkos_kirim_biaya > 0)
            ...
            <tr>
                ...
                <td class="text-right"><strong>Rp {{ number_format($preorder->subtotal, 0, ',', '.') }}</strong></td>
                <td></td>
            </tr>
            @if($preorder->total_additional_costs > 0)
            <tr>
                <td colspan="4" class="text-right"><strong>Total Biaya Tambahan:</strong></td>
                <td class="text-right"><strong>Rp {{ number_format($preorder->total_additional_costs, 0, ',', '.') }}</strong></td>
                <td></td>
            </tr>
            <tr>
                <td colspan="4" class="text-right"><strong>Subtotal:</strong></td>
                <td class="text-right"><strong>Rp {{ number_format($preorder->subtotal_with_additional_costs, 0, ',', '.') }}</strong></td>
                <td></td>
            </tr>
            @else
            <tr>
            ...
            <tr>
                ...
                <td class="text-right"><strong>-Rp {{ number_format($preorder->diskon, 0, ',', '.') }}</strong></td>
                <td></td>
            </tr>
            @endif
            @if($preorder->pajak > 0)
            <tr>
                ...
                <td class="text-right"><strong>Rp {{ number_format($preorder->pajak, 0, ',', '.') }}</strong></td>
                <td></td>
            </tr>
            @endif
            <tr>
                <td colspan="4" class="text-right"><strong>TOTAL:</strong></td>
                <td class="text-right"><strong>Rp {{ number_format($preorder->grand_total_with_additional_costs, 0, ',', '.') }}</strong></td>
                <td></td>
            </tr>
        </tbody>
    </table>
    ...
    @if($preorder->items->where('product_specifications', '!=', null)->count() > 0)
    <div class="page-break">
        ...
        @foreach($preorder->items as $item)
            ...
            <div class="spec-item">
                ...
                @if(is_array($item->product_specifications))
                    <ul class="spec-list">
                ...
                @else
                    <div>{!! nl2br(e($item->product_specifications)) !!}</div>
                @endif
            </div>
            @endif
        @endforeach
