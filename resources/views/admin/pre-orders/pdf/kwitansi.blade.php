...
<head>
    <meta charset="utf-8">
    <title>Kwitansi {{ $preorder->kode_preorder }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="kwitansi-number">
        No. {{ $preorder->kode_preorder }}/KWIT
    </div>
    ...
    <div class="content">
        <div class="content-row">
            ...
            <span class="content-value">{{ $preorder->customer->nama ?? '-' }}</span>
        </div>

        <div class="content-row">
            <span class="content-label">Uang sejumlah</span>
            <span class="content-colon">:</span>
            <span class="content-value">Rp {{ number_format($preorder->total, 0, ',', '.') }}</span>
        </div>

        <div class="amount-words">
            <strong>Terbilang:</strong> {{ $this->terbilang($preorder->total) }} Rupiah
        </div>

        <div class="content-row">
            <span class="content-label">Untuk pembayaran</span>
            <span class="content-colon">:</span>
            <span class="content-value">Pelunasan Pre Order {{ $preorder->kode_preorder }}</span>
        </div>
    </div>
    ...
    <div class="payment-details">
        ...
        <table class="payment-table">
            <tr>
                ...
                <td class="amount">Rp {{ number_format($preorder->total, 0, ',', '.') }}</td>
            </tr>
            @if($preorder->dp_amount)
            <tr>
                <td class="label">Down Payment (DP):</td>
                <td class="amount">Rp {{ number_format($preorder->dp_amount, 0, ',', '.') }}</td>
            </tr>
            <tr>
                <td class="label">Pelunasan:</td>
                <td class="amount">Rp {{ number_format($preorder->remaining_payment, 0, ',', '.') }}</td>
            </tr>
            @endif
            <tr style="border-top: 2px solid #000;">
                <td class="label"><strong>Total Diterima:</strong></td>
                <td class="amount"><strong>Rp {{ number_format($preorder->total, 0, ',', '.') }}</strong></td>
            </tr>
        </table>
    </div>
    ...
    <div class="signature-section">
        <div class="signature-box">
            ...
            <div><strong>{{ $preorder->customer->nama ?? '-' }}</strong></div>
    ...
    <div style="margin-top: 40px; text-align: center; font-size: 11px; color: #666; border-top: 1px solid #ccc; padding-top: 20px;">
        Kwitansi ini dicetak pada {{ now()->format('d/m/Y H:i:s') }}<br>
