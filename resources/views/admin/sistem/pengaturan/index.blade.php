<x-layouts.admin>
    <x-slot name="title">Pengaturan Perusahaan</x-slot>
    <x-slot name="header">Pengaturan Perusahaan</x-slot>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <select id="outlet_filter" onchange="changeOutlet()" 
                        ...
                        @foreach($outlets as $outlet)
                            <option value="{{ $outlet->id_outlet }}" {{ $outlet->id_outlet == $currentOutletId ? 'selected' : '' }}>
                                {{ $outlet->nama_outlet }}
                            </option>
                        @endforeach
                    </select>
                    <div class="text-sm text-slate-500">
                        <span class="inline-flex items-center px-2 py-1 bg-slate-100 rounded-full">
                            ...
                            Outlet Aktif: {{ $setting->outlet->nama_outlet ?? 'Tidak ada' }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-slate-900 font-medium">{{ $setting->company_name }}</p>
                    </div>

                    @if($setting->company_code)
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Kode Perusahaan</label>
                        <p class="text-slate-900">{{ $setting->company_code }}</p>
                    </div>
                    @endif
                    ...
                    <div>
                        ...
                        <p class="text-slate-900">{!! $setting->formatted_address !!}</p>
                    </div>
                    @endif
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-slate-900">{{ $setting->company_phone }}</p>
                        </div>
                        @endif
                        ...
                        <div>
                            ...
                            <p class="text-slate-900">{{ $setting->company_email }}</p>
                        </div>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        <a href="{{ $setting->company_website }}" target="_blank" class="text-primary-600 hover:text-primary-700">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        @if($setting->company_logo)
                            <div class="flex items-center space-x-4">
                                <img src="{{ $setting->logo_url }}" alt="Company Logo" class="h-16 w-auto object-contain border border-slate-200 rounded-lg p-2">
                                <div>
                                    <p class="text-sm text-slate-600">Logo saat ini</p>
                                    <p class="text-xs text-slate-500">{{ basename($setting->company_logo) }}</p>
                                </div>
                            </div>
                        @else
                    ...
                    <div>
                        ...
                        @if($setting->company_favicon)
                            <div class="flex items-center space-x-4">
                                <img src="{{ $setting->favicon_url }}" alt="Favicon" class="h-8 w-8 object-contain border border-slate-200 rounded">
                                <div>
                                    <p class="text-sm text-slate-600">Favicon saat ini</p>
                                    <p class="text-xs text-slate-500">{{ basename($setting->company_favicon) }}</p>
                                </div>
                            </div>
                        @else
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex justify-between items-center py-2 border-b border-slate-100">
                        ...
                        <span class="text-sm text-slate-900">{{ $setting->npwp }}</span>
                    </div>
                    @endif
                    ...
                    <div class="flex justify-between items-center py-2 border-b border-slate-100">
                        ...
                        <span class="text-sm text-slate-900">{{ $setting->nib }}</span>
                    </div>
                    @endif
                    ...
                    <div class="flex justify-between items-center py-2 border-b border-slate-100">
                        ...
                        <span class="text-sm text-slate-900">{{ $setting->siup }}</span>
                    </div>
                    @endif
                    ...
                    <div class="flex justify-between items-center py-2 border-b border-slate-100">
                        ...
                        <span class="text-sm text-slate-900">{{ $setting->tdp }}</span>
                    </div>
                    @endif

                    @if(!$setting->npwp && !$setting->nib && !$setting->siup && !$setting->tdp)
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex justify-between items-center py-2 border-b border-slate-100">
                        ...
                        <span class="text-sm text-slate-900">{{ $setting->bank_name }}</span>
                    </div>
                    @endif
                    ...
                    <div class="flex justify-between items-center py-2 border-b border-slate-100">
                        ...
                        <span class="text-sm text-slate-900 font-mono">{{ $setting->bank_account_number }}</span>
                    </div>
                    @endif
                    ...
                    <div class="flex justify-between items-center py-2 border-b border-slate-100">
                        ...
                        <span class="text-sm text-slate-900">{{ $setting->bank_account_name }}</span>
                    </div>
                    @endif

                    @if(!$setting->bank_name && !$setting->bank_account_number && !$setting->bank_account_name)
                    <div class="text-center py-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-slate-50 rounded-lg p-4">
                    ...
                    <p class="text-slate-900 font-medium">{{ $setting->currency }}</p>
                </div>

                <div class="bg-slate-50 rounded-lg p-4">
                    <label class="block text-sm font-medium text-slate-700 mb-1">Zona Waktu</label>
                    <p class="text-slate-900 font-medium">{{ $setting->timezone }}</p>
                </div>

                <div class="bg-slate-50 rounded-lg p-4">
                    <label class="block text-sm font-medium text-slate-700 mb-1">Format Tanggal</label>
                    <p class="text-slate-900 font-medium">{{ $setting->date_format }}</p>
                </div>

                <div class="bg-slate-50 rounded-lg p-4">
                    <label class="block text-sm font-medium text-slate-700 mb-1">Pajak (%)</label>
                    <p class="text-slate-900 font-medium">{{ number_format($setting->tax_rate, 2) }}%</p>
                </div>
            </div>
    ...
    <script>
        ...
        function removeCurrentLogo() {
            if (confirm('Apakah Anda yakin ingin menghapus logo perusahaan?')) {
                fetch('{{ route("admin.sistem.pengaturan.remove-file") }}', {
                    method: 'POST',
                    headers: {
        ...
        document.getElementById('companySettingsForm').addEventListener('submit', function(e) {
            ...
            // Send request
            xhr.open('POST', '{{ route("admin.sistem.pengaturan.update") }}');
