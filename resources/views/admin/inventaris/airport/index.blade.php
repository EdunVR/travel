<x-layouts.admin :title="'Travel / Bandara'">
  ...
  function airportCrud() {
    return {
      ...
      async fetchData() {
        this.loading = true;
        try {
          const res = await fetch(`{{ route('admin.inventaris.airport.data') }}?search=${encodeURIComponent(this.search)}`);
      ...
      async submitForm() {
        ...
        try {
          const url = this.form.id
            ? `{{ url('admin/inventaris/airport') }}/${this.form.id}`
            : `{{ route('admin.inventaris.airport.store') }}`;
      ...
      async deleteNow() {
        this.deleting = true;
        try {
          await fetch(`{{ url('admin/inventaris/airport') }}/${this.toDelete.id}`, {
            method: 'DELETE',
