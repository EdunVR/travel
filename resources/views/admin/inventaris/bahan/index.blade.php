                    ...
                    <tr class="hover:bg-slate-50">
                      ...
                      <td class="px-4 py-3 text-center">
                        <div class="flex justify-center gap-1">
                          @hasPermission('inventaris.bahan.edit-price')
                          <template x-if="editingPrice !== detail.id">
                          ...
                          @hasPermission('inventaris.bahan.edit-stock')
                          <template x-if="editingStock !== detail.id">
    ...
    function bahanCrud(){
      return {
        ...
        async submitStock() {
          ...
          try {
            const response = await fetch('{{ route("admin.inventaris.bahan.store-stock") }}', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        async savePrice(detailId) {
          ...
          try {
            const response = await fetch(`{{ route('admin.inventaris.bahan.update-price', '') }}/${detailId}`, {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        async saveStock(detailId) {
          ...
          try {
            const response = await fetch(`{{ route('admin.inventaris.bahan.update-stock', '') }}/${detailId}`, {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        async refreshHargaBeli() {
          ...
          try {
            const response = await fetch(`{{ route('admin.inventaris.bahan.edit', '') }}/${this.selectedBahan.id}`);
            const data = await response.json();
        ...
        async deleteHarga(detailId) {
          try {
            const response = await fetch(`{{ route('admin.inventaris.bahan.destroy_harga', '') }}/${detailId}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
              }
