<x-layouts.admin :title="'Master / Penerbangan'">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        <template x-if="activeTab === 'groups'">
          @hasPermission('master.flight.create')
              ...
              <tr class="border-t border-slate-100">
                ...
                <td class="px-4 py-3">
                  <div class="flex justify-end gap-2">
                    @hasPermission('master.flight.edit')
                    ...
                    @hasPermission('master.flight.delete')
      ...
      return {
        ...
        async fetchGroups(){
          ...
          try {
            ...
            const response = await fetch(`{{ route('admin.inventaris.flight-group.data') }}?${params}`);
        ...
        async fetchData(){
          ...
          try {
            ...
            const response = await fetch(`{{ route('admin.inventaris.flight.data') }}?${params}`);
            const data = await response.json();
        ...
        async fetchAirlines(){
          try {
            const response = await fetch('{{ route("admin.inventaris.flight.airlines") }}');
            const data = await response.json();
            this.airlines = data;
          } catch (error) {
        ...
        async fetchAirlineList(){
          try {
            const res = await fetch('{{ route("admin.inventaris.airline.list") }}');
        ...
        async fetchAirportList(){
          try {
            const res = await fetch('{{ route("admin.inventaris.airport.list") }}');
        ...
        async openEdit(f){ 
          try {
            const response = await fetch(`{{ route('admin.inventaris.flight.show', '') }}/${f.id}`);
            const data = await response.json();
        ...
        async submitForm(){
          ...
          try {
            const url = this.form.id 
              ? `{{ route('admin.inventaris.flight.update', '') }}/${this.form.id}`
              : '{{ route("admin.inventaris.flight.store") }}';
        ...
        async deleteNow(){
          ...
          this.deleting = true;
          try {
            const response = await fetch(`{{ route('admin.inventaris.flight.destroy', '') }}/${this.toDelete.id}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
        ...
        async openEditGroup(g) {
          try {
            const response = await fetch(`{{ route('admin.inventaris.flight-group.show', '') }}/${g.id}`);
            const data = await response.json();
        ...
        async submitGroupForm() {
          ...
          try {
            const url = this.groupForm.id 
              ? `{{ route('admin.inventaris.flight-group.update', '') }}/${this.groupForm.id}`
              : '{{ route("admin.inventaris.flight-group.store") }}';
        ...
        async deleteGroupNow() {
          ...
          this.deleting = true;
          try {
            const response = await fetch(`{{ route('admin.inventaris.flight-group.destroy', '') }}/${this.toDeleteGroup.id}`, {
              method: 'DELETE',
              headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
