<x-layouts.admin title="Kontra Bon">
    ...
    @include('admin.penjualan.kontrabon.modals.create')

    <!-- Include Print Modal -->
    @include('admin.penjualan.kontrabon.modals.print')
    ...
    <script>
        ...
        function kontraBonManagement() {
            return {
                ...
                selectedOutlets: @json($outlets->pluck('id_outlet')->toArray()), // Default: semua outlet terceklis
                ...
                getSelectedOutletsText() {
                    if (this.selectedOutlets.length === 0) {
                        ...
                        const outlet = @json($outlets).find(o => o.id_outlet == this.selectedOutlets[0]);
                ...
                selectAllOutlets() {
                    this.selectedOutlets = @json($outlets->pluck('id_outlet')->toArray());
                ...
                initPiutangTable() {
                    ...
                    tablePiutang = $('#table-piutang').DataTable({
                        ...
                        ajax: {
                            url: '{{ route("admin.penjualan.kontrabon.data") }}',
                            type: 'GET',
                            data: function(d) {
                ...
                initKontraBonTable() {
                    ...
                    tableKontraBon = $('#table-kontrabon').DataTable({
                        ...
                        destroy: false, // Never destroy
                        ajax: {
                            url: '{{ route("admin.penjualan.kontrabon.data-kontrabon") }}',
                            type: 'GET',
                            data: function(d) {
                                // Send outlet_ids as individual parameters for GET request
                                if (self.selectedOutlets && self.selectedOutlets.length > 0) {
        ...
        function showDetail(id) {
            ...
            // Load content
            $.get('{{ route("admin.penjualan.kontrabon.show", ":id") }}'.replace(':id', id))
                .done(function(data) {
        ...
        function showPrintModal(id) {
            ...
            window.open('{{ route("admin.penjualan.kontrabon.print", ":id") }}'.replace(':id', id), '_blank');
        }
        ...
        function lunasi(id) {
            if (confirm('Apakah Anda yakin ingin melunasi kontra bon ini? Semua piutang yang terlibat akan diubah statusnya menjadi lunas.')) {
                $.ajax({
                    url: '{{ route("admin.penjualan.kontrabon.lunasi", ":id") }}'.replace(':id', id),
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
        ...
        function deleteKontraBon(id) {
            if (confirm('Apakah Anda yakin ingin menghapus kontra bon ini? Status piutang yang terkait akan dikembalikan ke belum lunas.')) {
                $.ajax({
                    url: '{{ route("admin.penjualan.kontrabon.destroy", ":id") }}'.replace(':id', id),
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
