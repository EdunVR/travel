...
<div class="modal fade" id="modal-create-kontrabon" tabindex="-1" role="dialog" aria-labelledby="modal-create-kontrabon-label" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            ...
            <form id="form-create-kontrabon" action="{{ route('admin.penjualan.kontrabon.store') }}" method="POST">
                @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                @else
                                <input type="hidden" name="id_outlet" id="modal_id_outlet" value="{{ $outlets->first()->id_outlet ?? '' }}">
                                <label>Outlet</label>
                                <input type="text" class="form-control" value="{{ $outlets->first()->nama_outlet ?? 'No outlet' }}" readonly>
                                @endif
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="id_member" id="modal_id_member" class="form-control" required>
                                    ...
                                    @foreach ($members as $member)
                                        <option value="{{ $member->id_member }}" data-saldo="{{ $member->saldo ?? 0 }}">{{ $member->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
...
$(document).ready(function() {
    ...
    function modalFetchPiutang(idMember) {
        ...
        $.ajax({
            url: `{{ route('admin.penjualan.kontrabon.get-piutang', ':id') }}`.replace(':id', idMember),
            method: 'GET',
            success: function(response) {
