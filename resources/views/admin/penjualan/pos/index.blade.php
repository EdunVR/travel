  ...
  <section class="grid grid-cols-1 lg:grid-cols-5 gap-4">
    ...
    <div class="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-4 shadow-card flex flex-col">
      ...
      <div class="border-t pt-3">
        ...
        {{-- Form Tanggal Jatuh Tempo (muncul saat Bon dicentang) --}}
            ...
            <tbody class="divide-y divide-slate-200">
              <template x-for="item in historyData" :key="item.id">
                <tr class="hover:bg-slate-50">
                  ...
                  <td class="px-3 py-2 text-center">
                    {{-- Jika status BON, tampilkan tanggal jatuh tempo --}}
                    ...
                    {{-- Jika status LUNAS, tampilkan jenis pembayaran --}}
                    ...
                    {{-- Jika status BON tapi tidak ada data piutang --}}
                    <div x-show="item.status === 'menunggu' && (!item.piutang || !item.piutang.tanggal_jatuh_tempo)">
...
{{-- Initialize POS data for the separate pos.js file --}}
<script>
// Set up window variables for POS initialization
window.posInitialOutlet = {{ $selectedOutlet }};
window.posCSRFToken = '{{ csrf_token() }}';

// Set up route variables for POS API calls
window.posProductsRoute = '{{ route("admin.penjualan.pos.products") }}';
window.posCustomersRoute = '{{ route("admin.penjualan.pos.customers") }}';
window.posCustomerTypePricesRoute = '{{ route("admin.penjualan.pos.customer-type-prices") }}';
window.posAccountingBooksRoute = '{{ route("finance.accounting-books.data") }}';
window.posChartOfAccountsRoute = '{{ route("finance.chart-of-accounts.data") }}';
window.posCoaSettingsRoute = '{{ route("admin.penjualan.pos.coa.settings") }}';
window.posCoaSettingsUpdateRoute = '{{ route("admin.penjualan.pos.coa.settings.update") }}';
window.posStoreRoute = '{{ route("admin.penjualan.pos.store") }}';
window.posPrintRoute = '{{ route("admin.penjualan.pos.print", ":id") }}';
window.posHistoryRoute = '{{ route("admin.penjualan.pos.history.data") }}';
window.posDashboardRoute = '{{ route("admin.dashboard") }}';
window.posLoginRoute = '{{ route("login") }}';
