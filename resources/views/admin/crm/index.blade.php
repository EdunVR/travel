<x-layouts.admin :title="'CRM Dashboard'">
    ...
    <div x-data="crmDashboard({{ json_encode($userOutlets->pluck('id_outlet')->toArray()) }}, {{ $defaultOutlet }})" x-init="init()" class="space-y-6 overflow-x-hidden">
        {{-- Header & Filter --}}
        ...
        function crmDashboard(availableOutlets, defaultOutlet) {
            return {
                ...
                async loadData() {
                    ...
                    try {
                        // Load analytics
                        const analyticsUrl = `{{ route('admin.crm.dashboard.analytics') }}?period=${period}` + (outletParams ? '&' + outletParams : '');
                        ...
                        // Load predictions
                        const predictionsUrl = `{{ route('admin.crm.dashboard.predictions') }}` + (outletParams ? '?' + outletParams : '');
