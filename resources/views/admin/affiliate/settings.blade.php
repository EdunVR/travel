<x-layouts.admin :title="'Pengaturan Affiliate'">
<div class="space-y-6">
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-card p-6">
            ...
            <form action="{{ route('admin.inventaris.affiliate.settings.save') }}" method="POST" class="space-y-5">
                @csrf
                ...
                <div class="flex items-start justify-between gap-4">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer flex-shrink-0">
                        <input type="checkbox" name="{{ $t['key'] }}" value="1" class="sr-only peer"
                               {{ $t['checked'] ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <select name="cookie_lifetime"
                        ...
                        @endphp
                        @foreach($cookieOptions as $val => $label)
                        <option value="{{ $val }}" {{ $currentCookie == $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <div class="flex">
                        ...
                        <input type="number" name="default_click_commission"
                               value="{{ $settings['default_click_commission']->value ?? 1000 }}"
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <select name="default_sale_commission_type"
                            ...
                            <option value="percentage" {{ ($settings['default_sale_commission_type']->value ?? 'percentage') == 'percentage' ? 'selected' : '' }}>Persentase (%)</option>
                            <option value="flat"       {{ ($settings['default_sale_commission_type']->value ?? '') == 'flat' ? 'selected' : '' }}>Nominal (Rp)</option>
                        </select>
                        <input type="number" name="default_sale_commission_value"
                               value="{{ $settings['default_sale_commission_value']->value ?? 5 }}"
                ...
                <div>
                    ...
                    <div class="flex">
                        ...
                        <input type="number" name="minimum_payout"
                               value="{{ $settings['minimum_payout']->value ?? 100000 }}"
                               class="flex-1 h-9 px-3 rounded-r-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300"
        ...
        <div class="space-y-4">
            <div class="bg-white rounded-xl border border-slate-200 shadow-card p-6">
                ...
                <form action="{{ route('admin.inventaris.affiliate.package-commission.save') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            @forelse($packageCommissions as $pc)
                            <tr class="hover:bg-slate-50">
                                <td class="px-4 py-2.5 text-xs text-slate-700">{{ $pc->package->package_name ?? 'N/A' }}</td>
                                <td class="px-4 py-2.5 text-center text-xs">Rp {{ number_format($pc->click_commission, 0, ',', '.') }}</td>
                                <td class="px-4 py-2.5 text-center text-xs">
                                    {{ $pc->sale_commission_type === 'percentage' ? $pc->sale_commission_value.'%' : 'Rp '.number_format($pc->sale_commission_value, 0, ',', '.') }}
                                </td>
                                <td class="px-4 py-2.5 text-center">
                                    <form action="{{ route('admin.inventaris.affiliate.package-commission.delete', $pc) }}" method="POST">
                                        @csrf @method('DELETE')
