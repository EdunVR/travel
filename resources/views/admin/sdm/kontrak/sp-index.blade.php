<x-layouts.admin>
    <div class="p-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('sdm.kontrak.export.sp.pdf') }}" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition">
                    <i class="fas fa-file-pdf mr-2"></i>Export PDF
                </a>
                <a href="{{ route('sdm.kontrak.sp.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($sp as $item)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $item->nomor_sp }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $item->recruitment->name ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <span class="px-2 py-1 bg-red-100 text-red-800 rounded text-xs font-semibold">{{ $item->jenis_sp }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $item->tanggal_sp->format('d/m/Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $item->tanggal_berlaku->format('d/m/Y') }} - {{ $item->tanggal_berakhir ? $item->tanggal_berakhir->format('d/m/Y') : '-' }}
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-900 max-w-xs truncate">{{ $item->alasan }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <div class="flex gap-2 items-center">
                                    <!-- Print PDF Button -->
                                    <button onclick="openPrintModal({{ $item->id }}, '{{ addslashes($item->nomor_sp) }}')" 
                                    ...
                                    @endif
                                    <a href="{{ route('sdm.kontrak.sp.edit', $item->id) }}" class="text-yellow-600 hover:text-yellow-800" title="Edit"><i class="fas fa-edit"></i></a>
                                    <form action="{{ route('sdm.kontrak.sp.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus?')">
                                        @csrf
                                        @method('DELETE')
    ...
    <script>
        function openPrintModal(spId, nomorSp) {
            ...
            const pdfUrl = '{{ route("sdm.kontrak.sp.print", ":id") }}'.replace(':id', spId);
