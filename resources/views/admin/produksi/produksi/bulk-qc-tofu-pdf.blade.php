...
<body>
    ...
    <table class="main-table">
        ...
        <tbody>
            @foreach($qcData as $index => $qc)
            <tr class="data-row">
                <td class="text-center">{{ $index + 1 }}</td>
                <td class="text-center">{{ $qc['start_date'] }}</td>
                <td class="text-center">{{ $qc['production_code'] }}</td>
                
                <!-- Perendaman Kacang Kedelai -->
                <td class="text-center">{{ $qc['qc_data']['perendaman_waktu'] ?? '-' }}</td>
                <td class="text-center">{{ $qc['qc_data']['perendaman_kuantitas'] ?? ($qc['qc_data']['rijek_telur'] ?? '-') }}</td>
                
                <!-- Pasteurisasi -->
                <td class="text-center">{{ $qc['qc_data']['pasteurisasi_waktu'] ?? '-' }}</td>
                <td class="text-center">{{ $qc['qc_data']['pasteurisasi_suhu'] ?? '-' }}</td>
                
                <!-- Berat Adonan Pencampuran -->
                <td class="text-center">{{ $qc['qc_data']['pencampuran_waktu'] ?? ($qc['qc_data']['homogenisasi_waktu'] ?? '-') }}</td>
                <td class="text-center">{{ $qc['qc_data']['pencampuran_kuantitas'] ?? ($qc['qc_data']['target_quantity'] ?? '-') }}</td>
                
                <!-- Filling & Pengemasan -->
                <td class="text-center">{{ $qc['qc_data']['packaging_waktu'] ?? ($qc['qc_data']['filling_waktu'] ?? '-') }}</td>
                <td class="text-center">{{ $qc['qc_data']['mesin_1'] ?? ($qc['qc_data']['realized_quantity_1'] ?? '-') }}</td>
                <td class="text-center">{{ $qc['qc_data']['mesin_2'] ?? ($qc['qc_data']['realized_quantity_2'] ?? '-') }}</td>
                
                <!-- Total & Reject -->
                <td class="text-center">{{ $qc['qc_data']['total_kuantitas'] ?? ($qc['total_realized'] ?? '-') }}</td>
                <td class="text-center">{{ $qc['qc_data']['jumlah_reject_mentah'] ?? ($qc['rejected_quantity'] ?? '-') }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div style="margin-top: 20px; font-size: 7px; text-align: center; color: #666;">
        <p>Laporan dibuat pada: {{ $filters['export_date'] }}</p>
        <p>Total {{ $filters['total_count'] }} data QC Egg Tofu Mentah</p>
        @if($filters['outlet'] !== 'Semua Outlet')
