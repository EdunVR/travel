...
<body>
            ...
            <tr>
                <td class="logo-cell">
                    @if($companyLogo && file_exists(storage_path('app/public/' . $companyLogo)))
                        <img src="{{ storage_path('app/public/' . $companyLogo) }}" alt="Logo" style="width: 60px; height: 60px; object-fit: contain;">
                    @else
    ...
    <table class="main-table">
        ...
        <tbody>
            @foreach($qcData as $data)
            <tr>
                <td class="text-center">{{ $data['no'] }}</td>
                <td class="text-center">{{ $data['tanggal_produksi'] }}</td>
                <td class="text-center">{{ $data['kode_produk'] }}</td>
                
                <!-- Perendaman Kacang Kedelai -->
                <td class="text-center">{{ $data['perendaman_waktu'] }}</td>
                <td class="text-center">{{ $data['perendaman_kuantitas'] }}</td>
                
                <!-- Jumlah Reject Telur -->
                <td class="text-center">{{ $data['reject_telur_kuantitas'] }}</td>
                
                <!-- Pasteurisasi -->
                <td class="text-center">{{ $data['pasteurisasi_waktu'] }}</td>
                <td class="text-center">{{ $data['pasteurisasi_suhu'] }}</td>
                
                <!-- Berat Akhir Sari Kedelai -->
                <td class="text-center">{{ $data['berat_akhir_sari_kedelai'] }}</td>
                
                <!-- Pencampuran -->
                <td class="text-center">{{ $data['pencampuran_waktu'] }}</td>
                
                <!-- Filling & Pengemasan -->
                <td class="text-center">{{ $data['filling_waktu'] }}</td>
                <td class="text-center">{{ $data['filling_mesin_1'] }}</td>
                <td class="text-center">{{ $data['filling_mesin_2'] }}</td>
                <td class="text-center">{{ $data['total_kuantitas'] }}</td>
                
                <!-- Jumlah Reject Mentah -->
                <td class="text-center">{{ $data['jumlah_reject_mentah'] }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <!-- Footer -->
    <div class="footer">
        <p>Laporan dibuat pada: {{ $exportDate }}</p>
        <p>Total {{ $totalRecords }} data QC Egg Tofu Mentah</p>
    </div>
</body>
</html>