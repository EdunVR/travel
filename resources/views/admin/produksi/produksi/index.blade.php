<x-layouts.admin :title="'Produksi'">
  <style>
            ...
            <div>
              ...
              <select id="outletSelect" name="outlet_id" class="w-full rounded-xl border border-slate-200 px-3 py-2 focus:ring-2 focus:ring-primary-200" required>
                ...
                @foreach($outlets as $outlet)
                  <option value="{{ $outlet->id_outlet }}" {{ $selectedOutlet == $outlet->id_outlet ? 'selected' : '' }}>
                    {{ $outlet->nama_outlet }}
                  </option>
                @endforeach
              </select>
  ...
  <script>
    function productionCrud(){
      return {
        ...
        // Filters and search
        search: '',
        outletFilter: '{{ $selectedOutlet && $selectedOutlet !== "ALL" ? $selectedOutlet : "" }}', // Will be set to first outlet in init
        statusFilter: 'ALL',
        lineFilter: 'ALL',
        sortKey: 'created_at',
        sortDir: 'desc',
        ...
        async fetchData(){
          ...
          try {
            ...
            const url = `{{ route('admin.produksi.produksi.data') }}?${params}`;
        ...
        async fetchOutlets(){
          ...
          try {
            const outletsData = @json($outlets->map(function($outlet) {
        ...
        async fetchStats(){
          ...
          try {
            ...
            const url = `{{ route('admin.produksi.produksi.statistics') }}?outlet_id=${outletParam}`;
        ...
        viewProduction(production) {
          // Open PDF view
          const url = `{{ route('admin.produksi.produksi.pdf', ':id') }}`.replace(':id', production.id);
        ...
        async loadProductionForEdit(productionId) {
          try {
            const url = `{{ route('admin.produksi.produksi.edit', ':id') }}`.replace(':id', productionId);
            const response = await fetch(url);
            const data = await response.json();
        ...
        downloadProductionPdf(productionId) {
          const url = `{{ route('admin.produksi.produksi.pdf', ':id') }}`.replace(':id', productionId);
          window.open(url, '_blank');
        },

        downloadQcTofuPdf(productionId) {
          const url = `{{ route('admin.produksi.produksi.qc-tofu-pdf', ':id') }}`.replace(':id', productionId);
          window.open(url, '_blank');
        },
        ...
        async approveProduction(production) {
          if (confirm(`Apakah Anda yakin ingin menyetujui produksi ${production.production_code}?`)) {
            try {
              const url = `{{ route('admin.produksi.produksi.approve', ':id') }}`.replace(':id', production.id);
              const response = await fetch(url, {
                method: 'POST',
                headers: {
        ...
        async startProduction(production) {
          if (confirm(`Apakah Anda yakin ingin memulai produksi ${production.production_code}?`)) {
            try {
              const url = `{{ route('admin.produksi.produksi.start', ':id') }}`.replace(':id', production.id);
              const response = await fetch(url, {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
        ...
        async confirmCompleteProduction() {
          ...
          try {
            ...
            const url = `{{ route('admin.produksi.produksi.complete', ':id') }}`.replace(':id', this.currentProduction.id);
            const response = await fetch(url, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
        ...
        async deleteProduction(production) {
          if (confirm(`Apakah Anda yakin ingin menghapus produksi ${production.production_code}? Tindakan ini tidak dapat dibatalkan.`)) {
            try {
              const url = `{{ route('admin.produksi.produksi.destroy', ':id') }}`.replace(':id', production.id);
              const response = await fetch(url, {
                method: 'DELETE',
                headers: {
                  'Content-Type': 'application/json',
        ...
        async loadProductRealizationRows(productionId) {
          try {
            ...
            const response = await fetch(`{{ route('admin.produksi.produksi.show', ':id') }}`.replace(':id', productionId));
        ...
        exportBulkProductionPdf(){
          ...
          window.open(`{{ route('admin.produksi.produksi.export.bulk-production-pdf') }}?${params}`, '_blank');
        },
        ...
        exportQcTofuMentahPdf(){
          ...
          window.open(`{{ route('admin.produksi.produksi.export.qc-tofu-mentah-pdf') }}?${params}`, '_blank');
        },
        ...
        exportExcel(){
          ...
          window.open(`{{ route('admin.produksi.produksi.export.excel') }}?${params}`, '_blank');
        },
    ...
    function loadMonthlyCostsData(outletId) {
      ...
      fetch(`{{ route('admin.produksi.produksi.monthly-production-costs.data') }}?outlet_id=${outletId}`)
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (monthlyCostsForm) {
        monthlyCostsForm.addEventListener('submit', function(e) {
          ...
          fetch('{{ route("admin.produksi.produksi.monthly-production-costs.store") }}', {
            method: 'POST',
            body: formData,
            headers: {
      ...
      fetch(`{{ route('admin.produksi.produksi.monthly-production-costs.data') }}?outlet_id=${outletId}`)
        ...
        .then(data => {
          if (data.success && data.current && data.current.total_cost > 0) {
            ...
            fetch(`{{ route('admin.produksi.produksi.monthly-costs.get') }}?outlet_id=${outletId}&limit=1`)
              .then(response => response.json())
              .then(detailData => {
  ...
  <script src="{{ asset('js/production.js') }}?v={{ time() }}&debug=1" onerror="console.warn('production.js not found')"></script>
  ...
  <script>
    ...
    const updateUrl = "{{ route('admin.produksi.produksi.update', ':id') }}";
    ...
    const materialsFifoUrl = "{{ route('admin.produksi.produksi.materials.fifo', ':id') }}";
    ...
    const attendanceCountUrl = "{{ route('admin.produksi.produksi.attendance.count') }}";
    const hppPreviewUrl = "{{ route('admin.produksi.produksi.hpp.preview') }}";
    const pdfUrl = "{{ route('admin.produksi.produksi.pdf', ':id') }}";
